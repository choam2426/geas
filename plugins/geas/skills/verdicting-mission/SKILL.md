...
5. **Append the mission-verdict entry.** `geas mission-verdict append` is full-payload only (no inline flags), so stage the JSON body in .geas/tmp/ using the current client's file-write mechanism and pass `--file`. For the exact field list, run `geas schema template mission-verdicts --op append`.
   ```bash
