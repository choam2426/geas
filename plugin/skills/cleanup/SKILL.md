---
name: cleanup
description: Entropy scan — detect AI slop, unused code, convention drift. Records findings in .geas/missions/{mission_id}/evolution/debt-register.json. Invoke after Building or during Evolving.
---
...
For each finding, add a structured debt item to `.geas/missions/{mission_id}/evolution/debt-register.json`. Read the existing file, append to the `items` array, update rollups, and write back.
...
```json
{
  ...
  "introduced_by_task_id": "<task-id or 'polishing'>",
...
Tech debt items recorded in .geas/missions/{mission_id}/evolution/debt-register.json
```
