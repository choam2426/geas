...
test('task contract record stores numbered contract and creates task-state', () => {
  ...
  assert.ok(existsSync(join(taskBase, 'task-contract-001.md')));
...
test('task contract record creates a new task id', () => {
  const result = runTaskContractRecord('task-999', taskContract('task-999'));
  assert.equal(result.ok, true);
...
test('task contract record rejects mismatched task id', () => {
  const result = runTaskContractRecord('task-001', taskContract('task-999'));
  assert.equal(result.ok, false);
  if (!result.ok) {
    assert.equal(result.error.code, 'frontmatter_mismatch');
  }
});

test('task contract record rejects unknown dependency', () => {
  const result = runTaskContractRecord('task-001', taskContract('task-001', ['task-999']));
  assert.equal(result.ok, false);
  if (!result.ok) {
...
test('task contract record rejects dependency cycle', () => {
  ...
  const second = runTaskContractRecord('task-002', taskContract('task-002', ['task-001']));
  assert.equal(second.ok, true);
  const cycle = runTaskContractRecord('task-001', taskContract('task-001', ['task-002']));
...
const verifEv = (verdict: 'passed' | 'changes_requested' | 'escalated') => verificationEvidence(verdict);
