 ...
 *
 * Asserts the v3 surfaces (src/cli/, plugins/geas/skills/,
 * plugins/geas/agents/, plugins/geas/hooks/) contain no occurrences of terminology that was retired
...
// invocation patterns are forbidden everywhere in plugins/geas/skills/, not
...
test('T3.1 / AC4: no `cat <<EOF` heredoc invocations remain in plugins/geas/skills/', () => {
  const files = listSkillMarkdown();
  assert.ok(files.length > 0, 'expected at least one SKILL.md / reference under plugins/geas/skills/');
  ...
  assert.deepEqual(
    findings,
    [],
    `legacy heredoc invocations remain in plugins/geas/skills/:\n${findings
      .map((f) => `  ${f.file}:${f.line}  ${f.text}`)
      .join('\n')}`,
  );
});

test('T3.1 / AC4: no `cat <file>.json | geas` stdin-pipe invocations remain in plugins/geas/skills/', () => {
  const files = listSkillMarkdown();
  assert.ok(files.length > 0, 'expected at least one SKILL.md / reference under plugins/geas/skills/');
  ...
  assert.deepEqual(
    findings,
    [],
    `legacy stdin-pipe invocations remain in plugins/geas/skills/:\n${findings
      .map((f) => `  ${f.file}:${f.line}  ${f.text}`)
      .join('\n')}`,
  );
});
