 ...
 *
 * After T3.2 migration, plugins/geas/skills/<name>/SKILL.md bodies must NOT
 ...
 *   - plugins/geas/skills/*\/references/*.md is allowed to contain inline JSON
...
test('T3.2 / AC6: no multi-line inline JSON template blocks remain in plugins/geas/skills/*/SKILL.md', () => {
  const files = listSkillMdBodies();
  assert.equal(
    files.length,
    17,
    `expected exactly 17 SKILL.md files under plugins/geas/skills/, got ${files.length}`,
  );
