...
| `geas schema validate --type <t>` | 지정한 schema로 JSON payload(`--file` 또는 stdin)를 검증 |
| `geas context` | 현재 mission / task 상태를 agent bootstrap용 context JSON으로 반환 |
| `geas state get` | 활성 mission의 phase와 각 task 상태를 structured JSON으로 반환 |
...
JSON envelope를 소비하는 skill·agent는 `--json`을 명시한다. default scalar 모드는 사람 대상 reader (그리고 `plugins/geas/skills/mission/references/briefing-templates.md`의 사용자 대면 브리핑 layer)를 겨냥한 surface다 — 프로그램 소비자는 default shape를 가정하지 말 것 (명령이 진화하면서 더 짧은 phrasing으로 drift 허용).
...
- 단일 payload는 `geas schema validate --type <t>`로 대응 schema에 대해 검증할 수 있다.
...
- `.geas/missions/{mid}/consolidation/candidates.json` 생성 (덮어쓰기 허용):
  ...
  ```
- Schema 검증 대상이 아님 (`candidates.json`은 지원용 파일).
...
- 이벤트 기록은 **best-effort**다. 주 artifact 쓰기는 atomic하며 권위 있는 결과다. `events.jsonl` append는 그 뒤에 실행되고, 여기서 실패하더라도(디스크 full, 일시적 permission 오류 등) 주 쓰기를 rollback하지 않도록 삼킨다. 명령은 여전히 `ok`를 보고한다. `events.jsonl`을 transaction log처럼 소비하는 쪽은 이를 감안해야 한다 — source of truth는 telemetry가 아니라 디스크의 protocol artifact다.

### 14.7 Memory markdown 쓰기
...
CLI는 agent UX surface의 절반이다. 나머지 절반은 skill layer (`plugins/geas/skills/`)이며, 디스패처가 각 phase를 구동할 때 읽는다. 아래 규약들은 skill layer에 살지만 §1-14의 CLI 보장과 깔끔하게 맞물리기 위해 존재한다 — CLI 중심 reader가 skill 본문에서 이들을 만났을 때 어디를 봐야 할지 안내한다.
...
`plugins/geas/skills/drafting-task/SKILL.md`와 `plugins/geas/skills/specifying-mission/SKILL.md`는 task contract를 승인하기 전에 drafter가 수행해야 하는 사전 점검 의식을 담는다.
...
`plugins/geas/skills/mission/SKILL.md`는 orchestrator가 mission state machine을 routing할 때 읽는 디스패치 표를 담는다. 본 CLI 문서와 맞물리는 행은:
...
4종 사용자 대면 브리핑(`current-status`, `task-completion`, `phase-transition`, `mission-verdict`)은 한국어 narrative prose로 emit되며 field-colon JSON dump 형태가 아니다. 이들이 쓸 수 있는 어휘는 `plugins/geas/skills/mission/references/briefing-templates.md`에 jargon 화이트리스트로 enumerate되어 있다 — 브리핑으로 흘러 들어가는 CLI emit text는 이 화이트리스트를 존중해야 한다 (핵심 protocol 용어 `phase`, `gate`, `task`, `mission`, `evidence`, `verdict`는 허용; `tier`, `verify-fix`, `rewind` 같은 더 깊은 jargon은 비허용). CLI의 default scalar 출력 모드 (§5)가 이 layer에 feed되는 surface다.
