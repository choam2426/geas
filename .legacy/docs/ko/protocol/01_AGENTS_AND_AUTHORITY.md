...
프로토콜 전체에서 다음 표기를 일관되게 쓴다.

- **Slot identifier**: kebab-case. 예: `decision-maker`, `design-authority`, `risk-assessor`. Schema의 slot enum, task contract의 `required_reviewers` 값, evidence의 `slot` 필드와 파일 경로 `evidence/{agent}.{slot}.json` 모두 이 표기를 쓴다.
- **Concrete agent type**: kebab-case. 예: `software-engineer`, `security-engineer`. Authority slot의 type은 slot 이름과 동일(`decision-maker` 등).
- **Field enum value와 event actor**: slot id와 연결된 값도 **kebab-case를 유지한다**. 예: `approved_by: "decision-maker"`, `events.actor: "decision-maker"`. 프로토콜 안에서 같은 개념이 두 표기로 분기되지 않는다.
