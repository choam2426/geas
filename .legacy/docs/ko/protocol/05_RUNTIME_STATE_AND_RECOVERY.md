...
| `active_tasks` | `implementing` / `reviewing` / `deciding` 중 하나인 task ID 목록. 즉 아직 passed/cancelled/escalated로 종결되지 않았고 blocked로 멈추지도 않은 "진행 중 open work" (0개: idle, 1개: sequential, N개: parallel batch) |
| `created_at`, `updated_at` | 생성·갱신 시각 |

이미 종결된 task 목록, recovery 유형, 구현체의 scheduler 상태 같은 신호는 mission state에 담지 않는다. 종결 여부는 각 task의 `task-state.json` status로 파생하고, 구현체 신호는 프로토콜 관심사가 아니다.
...
Task state는 이 task의 lifecycle status, 지금 어떤 agent로 실행 중인지, verify-fix를 몇 번 거쳤는지를 담는 task 단위 runtime state다. Task contract는 "무엇을 해야 하는가"(intent + 승인)를 고정하고, task state는 "지금 어디까지 왔는가"(runtime 진행)를 담는다.
...
| `status` | 현재 task의 lifecycle 단계. 9-state 기계의 현재 상태 (drafted / ready / implementing / reviewing / deciding / passed / blocked / escalated / cancelled) |
...
- 파일 쓰기의 원자성은 CLI가 atomic rename으로 보장한다. task state가 별도 write 상태 플래그를 가지지 않는다.
