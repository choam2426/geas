...
Geas는 **slot 기반 역할 아키텍처**를 사용한다. 프로토콜이 추상적인 **slot**을 정의하고, 구체 agent 타입은 경로와 role body로 catalog화되며 task routing이 런타임에 concrete type을 slot에 바인딩한다. 이 분리 덕분에 동일한 거버넌스 파이프라인이 contract 계층 수정 없이 소프트웨어 엔지니어링, 연구, 앞으로 추가될 어떤 profile에도 적용된다.

**정식 slot 정의:** [`docs/protocol/01_AGENTS_AND_AUTHORITY.md`](../../protocol/01_AGENTS_AND_AUTHORITY.md)
**Agent 파일:** `plugins/geas/agents/`
...
**Orchestrator**는 미션 skill(메인 세션 드라이버)이지 스폰 가능한 agent가 아니다. Phase를 조율하고, specialist를 디스패치하고, task 종결 결정을 기록한다. `plugins/geas/agents/`에는 등장하지 않는다.
...
소프트웨어 엔지니어링 미션용. `plugins/geas/agents/software/`에 정의되어 있다.
...
연구·분석 미션용. `plugins/geas/agents/research/`에 정의되어 있다.
...
2. **Specialist slot**은 `plugins/geas/agents/software/` 또는 `plugins/geas/agents/research/` 아래의 domain catalog에서 채운다. 오케스트레이터는 mission profile과 task routing에 따라 slot에 들어갈 concrete agent type을 고른다. Agent body는 보통 맡는 slot 책임을 설명하지만, slot과 domain은 frontmatter 필드가 아니다.
...
| 구현 접근법(plan) | Implementer 단독. `implementing` 상태에서 implementation contract를 작성하며, reviewer는 작업 이후에 본다 |
