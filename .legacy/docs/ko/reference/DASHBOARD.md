...
- `geas context` / `geas state get` 대체제. 저건 단발 JSON, dashboard는 상호작용형 집계 뷰다.
...
**집계.** Task 수는 9-state lifecycle(`drafted`, `ready`, `implementing`, `reviewing`, `deciding`, `passed`, `blocked`, `escalated`, `cancelled`)로 그룹화.
...
- Task · evidence 편집 없음.
- 스크립트용 상태 조회(`geas context`, `geas state get`)의 대체가 아니다 — dashboard는 사람용이다.
- 스케줄링된 보고서 · 이메일 요약 없음.
