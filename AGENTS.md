...
- Caller는 role output을 직접 작성하지 않는다. Role-producing 단계는 먼저 `role_required`, `role_optional`, `role_omitted` 중 하나를 결정하고, 필요하면 handoff result를 기다린다.
...
- 주요 실패 패턴은 pre-scan 직후 Intake Sketch 없이 Mission Spec Review로 이동, work-designer decision 없이 Mission Design Review 작성, role-required Design 뒤 coordinator가 Task Contract를 작성, building context가 role Evidence를 직접 작성, role handoff 불가 시 caller가 role output을 대신 작성하는 것이다.
