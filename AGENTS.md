...
- 주요 실패 패턴은 pre-scan 직후 Intake Sketch 없이 Mission Spec Review로 이동, work-designer decision 없이 Mission Design Review 작성, building context가 role Evidence를 직접 작성, role handoff 불가 시 caller가 role output을 대신 작성하는 것이다.
