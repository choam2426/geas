...
|User Judgment|User가 Mission 기준선과 initial Task Contract, Task 결과, Mission 결과를 검토한 뒤 내린 결정을 남긴다.|
...
Mission Design의 `task_breakdown`은 Mission 수준 planned task graph의 정본이다. Task 추가, 삭제, 기본 진행 순서 변경, dependency 변경, mission coverage 변경은 Mission Design 갱신으로 다룬다.
...
|Key|Type|내용|
|---|---|---|
|`phase`|string|`contracting`, `implementing`, `verifying`, `reviewing`, `challenging`, `awaiting_user_judgment`, `summarizing` 중 하나|

Task State는 Task Contract, Evidence, User Judgment, Task Evidence를 대체하지 않는다. Task State는 완료 상태나 수용 판단이 아니라 현재 Task를 재개할 때 어느 phase부터 이어갈지 찾기 위한 artifact다.
...
User Judgment는 User가 Mission 기준선과 initial Task Contract, Task 결과, Mission 결과를 검토한 뒤 내린 결정을 남긴다.
...
`accepted`와 `accepted_with_limits`는 기준선이나 결과를 수용한 판단이다. `revise`는 현재 기준선이나 결과를 그대로 수용하지 않고, 재작업, Task Contract 갱신, 추가 Task, Mission 기준선 재검토, 폐기 중 필요한 조치를 요청하는 판단이다. 구체 조치는 `requested_actions`에 남긴다.
...
파일 하나는 하나의 Runtime artifact다. 반복 생성될 수 있는 Evidence와 결과 User Judgment는 파일명 끝의 번호를 증가시켜 쌓는다.

Mission Spec, Mission Design, Task Contract는 current 파일 하나를 유지한다.

`mission-spec.yaml`, `mission-design.yaml`, `user-judgment-baseline.yaml`은 하나의 현재 Mission 기준선 set이다. `user-judgment-baseline.yaml`은 현재 Mission Spec, Mission Design, building 진입에 필요한 initial Task Contract를 함께 판단한다.

`user-judgment-baseline.yaml`은 specifying에서 building으로 넘어갈 수 있다는 User 수용 판단이다.

`task-contract.yaml`은 현재 Task 실행 기준선이다.

`task-state.yaml`은 현재 Task phase pointer다. Task scope 안에서 current 파일 하나를 유지한다.

Mission Spec, Mission Design, Task Contract가 갱신되면 같은 파일을 갱신하고, 기준선이 바뀐 이유와 영향은 `contract_deltas`, `mission_design_deltas`, 종료 요약에 남긴다.
...
- Task State는 완료 상태, Evidence verdict, User Judgment가 아니다.
