...
|User Judgment|User가 Mission 기준선, Task 결과, Mission 결과를 검토한 뒤 내린 결정을 남긴다.|
...
Task Contract의 범위, 산출물, 수용 기준, verification checks, review focus가 바뀌면 실행 기준이 바뀐 것이다. 이 경우 갱신된 Task Contract를 새 번호로 남긴다.
...
`mission-spec.yaml`, `mission-design.yaml`, `user-judgment-baseline.yaml`은 하나의 현재 Mission 기준선 set이다. `user-judgment-baseline.yaml`은 현재 Mission Spec과 Mission Design을 함께 판단한다.
