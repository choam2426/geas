...
- artifact의 정본 파일 형식
- artifact별 frontmatter, Markdown section, YAML schema
- `.geas/` storage 구조
...
- Frontmatter가 필요한 artifact는 runtime이 색인, 상태 전이, 참조에 쓰는 metadata만 둔다.
...
다음 artifact는 YAML 파일이다.

- Run State
- Task State

Run State와 Task State는 진행 위치를 찾기 위한 pointer artifact다. 완료나 수용 판단의 근거는 User Judgment와 Evidence에 둔다.
...
Task State는 Task 안에서 현재 이어갈 Task Workflow 단계를 찾는 pointer artifact다.

Task State는 YAML 형식으로 저장한다.

|Key|Type|내용|
|---|---|---|
|`phase`|string|현재 Task phase|
...
`summarize`는 별도 phase로 두지 않는다. Task 요약이 준비되어 User 판단을 기다리는 상태는 `awaiting_user_judgment`로 표현한다.

Task State는 복귀 위치를 찾는 색인이다. 복귀 지점과 다음 행동은 Task State만으로 정하지 않고 `Task Contract`, Evidence, `User Judgment`, `Task Evidence`, `Task Memory`, `Continuity Ledger`를 대조해 정한다.
...
Run State는 YAML 형식으로 저장한다.

|Key|Type|내용|
|---|---|---|
|`current_mission_id`|string|현재 Mission id. 현재 Mission이 없으면 `""`|
|`current_stage`|string|`""`, `specifying`, `building`, `consolidating` 중 하나|
|`current_task_id`|string|현재 Task id. 없으면 `""`|

Run State는 작업 재개의 출발점이다. 복귀 지점은 Run State만으로 확정하지 않고 Mission Spec, Mission Plan, Task Direction, Task Contract, Evidence, User Judgment, Task Evidence, Mission Evidence, Task Memory, Debt Ledger, Memory, Continuity Ledger를 대조해 정한다.
...
Task Evidence, Mission Evidence, Task State, Run State, Debt Ledger, Task Memory, Memory, Continuity Ledger는 고정 파일명을 쓴다.
...
- Run State와 Task State는 진행 위치 pointer다.
