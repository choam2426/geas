...
Debt Ledger는 User가 알고 수용했고 이후 프로젝트 유지 비용으로 남는 debt item을 프로젝트 수준에서 추적한다.
...
Debt Ledger는 프로젝트 수준 debt item 목록이므로 YAML 형식으로 둔다.
...
|Mission Evidence|Mission 종료 후 Mission 결과, User의 Mission 수용 판단, 주요 Evidence, gap, Debt Ledger 반영, follow-up, 회고 요약, 반영한 Memory를 종합한다.|
|User Judgment|User가 Task 결과나 Mission 결과를 검토한 뒤 내린 결정을 남긴다.|
|Debt Ledger|User가 알고 수용했고 이후 프로젝트 유지 비용으로 남는 debt item을 프로젝트 수준 목록으로 추적한다.|
...
Mission Evidence는 User가 role Evidence, Task Evidence, gap, Debt Ledger 반영 후보, follow-up 후보를 검토해 User Judgment를 남긴 뒤 작성하는 Mission 종료 요약이다.
...
|`Debts`|현재 Mission에서 새로 기록하거나 갱신한 Debt Ledger item 참조와 짧은 요약|
...
Mission Evidence의 Debts는 debt의 정본이 아니다. 프로젝트 수준에서 계속 추적해야 하는 debt는 Debt Ledger에 기록하고, Mission Evidence에는 이번 Mission에서 새로 기록하거나 갱신한 debt id와 짧은 요약만 남긴다.
...
Debt Ledger는 User가 알고 수용했고 이후 프로젝트 유지 비용으로 남는 debt item 목록이다.

Debt Ledger는 YAML 형식으로 저장한다.

|Key|Type|내용|
|---|---|---|
|`items`|list of object|프로젝트 수준으로 추적하는 debt item 목록|

`items` 각 항목은 다음 schema를 따른다.

|Key|Type|내용|
|---|---|---|
|`debt_id`|string|프로젝트 안에서 debt item을 식별하는 id. `DEBT-001` 형식을 쓴다.|
|`status`|string|`open`, `resolved`, `dropped` 중 하나|
|`title`|string|debt를 빠르게 식별할 수 있는 짧은 이름|
|`summary`|string|무엇을 받아들였고 어떤 비용이 남는지에 대한 요약|
|`impact`|string|이 debt가 이후 변경, 유지보수, 검증, 운영에 만드는 비용|
|`source_refs`|list of string|debt 후보를 드러낸 Evidence, Task Evidence, Mission Evidence 참조|
|`accepted_in_ref`|string|User가 debt로 받아들인 판단을 담은 User Judgment 또는 Mission Evidence 참조|
|`revisit_when`|list of string|이 debt를 다시 봐야 하는 조건|
|`resolved_by_refs`|list of string|debt를 해결했거나 더 이상 적용되지 않게 만든 artifact 참조|

Debt Ledger는 Mission scope에 종속되지 않는 프로젝트 수준 artifact다. Mission이나 Task에서 드러난 debt 후보는 User가 비용과 위험을 알고 받아들였을 때 Debt Ledger에 기록하거나 기존 item을 갱신한다.

Debt item은 Memory item이 아니다. Memory는 이후 작업 방식에 반복 적용할 운영 지식이고, Debt는 현재 결과 안에 남아 이후 프로젝트 비용을 만드는 항목이다.

Debt item은 Follow Up이 아니다. Follow Up은 현재 Mission 밖에서 새로 다룰 수 있는 후보 작업이고, Debt는 현재 수용한 결과 안에 남아 비용을 만드는 항목이다.

Debt Ledger file 하나가 Runtime artifact이며, Debt item은 Runtime artifact가 아니다.
...
Mission Design과 Task Contract 작성 중 생긴 판단 맥락은 별도 설계 전용 Memory가 아니라 해당 기준선 artifact, Orchestrator Memory 후보, Debt Ledger와 Memory 반영 흐름에 남긴다.
...
Run State는 작업 재개의 출발점이다. 복귀 지점은 Run State만으로 확정하지 않고 기준선, Evidence, User Judgment, Mission Evidence, Debt Ledger, Memory를 대조해 정한다.
...
`<mission-id>`, `<task-id>`, Debt id는 Runtime artifact가 Mission, Task, 프로젝트 debt를 안정적으로 찾기 위한 id다.
...
같은 Mission id 디렉토리가 이미 있으면 Mission id의 random 값을 다시 생성한다. 새 Task id는 Orchestrator가 해당 Mission의 `tasks/` 아래에서 가장 큰 Task 번호에 1을 더해 정하고, `task contract record --task <task-id>`로 기록한다.

새 Debt id는 `.geas/debts.yaml`의 기존 item 중 가장 큰 Debt 번호에 1을 더해 정한다.

Run State는 `.geas/run-state.yaml`에 둔다.

Debt Ledger는 `.geas/debts.yaml`에 둔다.
...
종료 요약, Task State, Run State, Debt Ledger, Memory는 고정 파일명을 쓴다.

Mission scope 안의 artifact 참조는 `.geas/missions/<mission-id>/` 기준 상대 경로를 기본으로 쓴다. 같은 Task scope 안의 artifact를 참조할 때는 Task directory 기준 파일명을 쓸 수 있다. 기준선 artifact를 참조할 때는 versioned filename을 쓴다. 예: `mission-spec-001.md`, `tasks/task-001/task-contract-001.md`, `tasks/task-001/challenger-evidence-001.md`, `tasks/task-001/task-evidence.md`, `verification-evidence-002.md`.

Debt Ledger와 Memory처럼 Mission scope 밖의 프로젝트 수준 artifact가 다른 artifact를 참조할 때는 `.geas/` 기준 상대 경로를 쓴다. 예: `missions/20260507-a1b2c3/mission-evidence.md`, `missions/20260507-a1b2c3/tasks/task-001/review-evidence-001.md`.
...
- User가 받아들인 debt는 Mission에 묻히지 않고 Debt Ledger에서 프로젝트 수준으로 추적한다.
- Mission Evidence는 User가 Mission 수용 판단을 남긴 뒤 gap, Debt Ledger 반영, follow-up, 회고, Memory 반영 결과를 종합한다.
