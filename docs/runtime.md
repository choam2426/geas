...
artifact 참조는 `.geas/missions/<mission-id>/` 기준 상대 경로를 기본으로 쓴다. 같은 Task scope 안의 artifact를 참조할 때는 Task directory 기준 파일명을 쓸 수 있다. 기준선 artifact를 참조할 때는 versioned filename을 쓴다. 예: `mission-spec-001.yaml`, `tasks/task-001/task-contract-001.yaml`, `challenger-evidence-001.yaml`, `tasks/task-001/task-evidence.yaml`, `verification-evidence-002.yaml`.
