...
|Task Contract|Task 분리와 Task의 설명, Mission과의 관계, 의존 관계, 범위, 산출물, 수용 기준, verification checks, review focus, 위험 수준을 고정한다.|
...
|Key|Type|내용|
|---|---|---|
|`plan_summary`|string|Mission을 어떤 계획으로 진행할지에 대한 짧은 요약|
...
|`plan_outline`|list of object|Task로 쪼개기 전, User가 이해해야 할 주요 진행 초점과 기대 결과|
|`decision_points`|list of string|진행 중 User에게 돌아가야 하거나 기준선 재검토가 필요한 판단 지점|
|`assumptions`|list of string|작업 계획이 의존하는 전제|
|`risks`|list of string|선택한 Mission 진행 계획에서 고려해야 하는 위험|
|`change_triggers`|list of string|Mission Design이나 Task Contract를 다시 합의해야 하는 조건|
...
`plan_outline` 각 항목은 다음 schema를 따른다.

|Key|Type|내용|
|---|---|---|
|`focus`|string|계획에서 User가 먼저 이해해야 할 진행 초점|
|`purpose`|string|그 초점이 Mission 목표와 수용 판단 비용에 기여하는 이유|
|`user_visible_result`|string|그 초점이 진행되었을 때 User가 보게 될 결과나 판단 자료|

`plan_outline`은 Task 목록, Task id, dependency를 정의하지 않는다. 그 책임은 Task Contract에 둔다.
...
Task Contract는 Task 분리와 개별 Task 실행 계약의 정본이다. Task id, dependency, Task별 scope와 mission coverage는 Task Contract 기록으로 확정한다.
...
Task 추가, 삭제, dependency, 실행 범위, 산출물, 수용 기준, verification checks, review focus, risk_level이 바뀌면 Task Contract를 갱신한다.
...
|Key|Type|내용|
|---|---|---|
|`target`|list of string|압박한 Task Contract, 산출물, Evidence|
...
|`basis`|string|finding을 뒷받침하는 Task Contract, 산출물, Evidence, 관찰 근거|
...
|---|---|---|
|`criterion`|string|Task Contract의 acceptance criterion 원문 또는 참조|
|`result`|string|`satisfied`, `satisfied_with_limits`, `not_satisfied` 중 하나|
...
Mission Spec, Mission Design, Task Contract는 versioned 기준선 artifact다. `mission-spec-NNN.yaml`, `mission-design-NNN.yaml`, `task-contract-NNN.yaml`처럼 파일명 끝의 번호를 증가시켜 남긴다.

기준선 draft는 Runtime artifact가 아니다. Runtime storage에 남은 Mission Spec, Mission Design, Task Contract는 User와 합의된 기준선으로 본다.

현재 Mission 기준선은 해당 Mission scope에서 가장 큰 번호의 `mission-spec-NNN.yaml`과 `mission-design-NNN.yaml`이다.

현재 Task 실행 기준선은 해당 Task scope에서 가장 큰 번호의 `task-contract-NNN.yaml`이다.
...
artifact 참조는 `.geas/missions/<mission-id>/` 기준 상대 경로를 기본으로 쓴다. 같은 Task scope 안의 artifact를 참조할 때는 Task directory 기준 파일명을 쓸 수 있다. 기준선 artifact를 참조할 때는 versioned filename을 쓴다. 예: `mission-spec-001.yaml`, `tasks/task-001/task-contract-001.yaml`, `tasks/task-001/challenger-evidence-001.yaml`, `tasks/task-001/task-evidence.yaml`, `verification-evidence-002.yaml`.
