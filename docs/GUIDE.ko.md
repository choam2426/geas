...
Initiative 모드는 새로운 제품을 처음부터 만들기 위한 것입니다. Genesis, MVP Build, Polish, Evolution의 네 단계를 거칩니다.
...
이것만으로 충분합니다. Compass는 이것이 새로운 제품임을 감지하고 자동으로 Initiative 모드를 활성화합니다. 단계별로 다음과 같이 진행됩니다.
...
Compass가 브레인스토밍 스타일로 한 번에 하나씩 질문합니다. 숫자 척도로 답변을 채점하는 대신, Intake는 **완전성 체크리스트**를 사용하여 어떤 영역이 다뤄졌는지 추적합니다:
...
각 질문은 이전 답변을 기반으로 합니다. 체크리스트가 충분히 완성되면 Compass가 확인을 위한 요약을 생성합니다.
...
`/geas:initiative`, `/geas:sprint`, `/geas:debate`로 직접 모드를 지정할 수도 있습니다.
...
7. **Critic 출시 전 리뷰 (Critic)** -- Nova가 판정을 내리기 전에 Critic이 축적된 증거를 검토하고 의심스러운 것에 이의를 제기합니다: 검증되지 않은 가정, 누락된 엣지 케이스, 너무 쉽게 통과한 수용 기준. Critic의 피드백은 `.geas/evidence/{task-id}/critic-review.json`에 기록됩니다.
...
.geas/
  ...
  evidence/task-001/critic-review.json           -- Critic의 출시 전 리뷰
...
| Intake 깊이 | 깊음 (브레인스토밍 스타일, 전체 체크리스트) | 가벼움 (기능 범위와 제약에 집중) |
...
7. **Evidence Gate** -- Initiative과 동일한 3단계 게이트 (기계적, 의미적, 제품).

7.5. **Critic 출시 전 리뷰 (Critic)** [필수] -- Nova가 판정을 내리기 전에 Critic이 축적된 증거를 검토하고 의심스러운 것에 이의를 제기합니다.
...
.geas/
  ...
  evidence/
    ...
    task-001/
      ...
      critic-review.json            -- Critic의 출시 전 리뷰
...
**`spec/seed.json`** -- Intake Gate에서 생성되는 동결된 미션 사양입니다. 정제된 미션 선언, 대상 사용자, 범위(포함 및 제외), 수용 기준, 제약 조건, 완전성 체크리스트를 포함합니다. 확인 후에는 수정하면 안 됩니다. 범위 변경이 필요하면 대신 피벗 프로토콜이 트리거됩니다.
...
**`state/run.json`** -- 현재 실행 상태입니다. 미션 이름, 현재 모드(initiative/sprint/debate), 현재 단계(genesis/mvp/polish/evolve), 현재 작업 ID, 완료된 작업 목록, 전체 상태를 포함합니다. 세션 재개를 가능하게 하는 파일입니다.
...
| Hook | 실행 시점 | 동작 |
|------|-----------|------|
| **session-init** | 세션 시작 (`SessionStart`) | `run.json`과 체크포인트에서 컨텍스트 복원, `rules.md`가 없으면 생성, 상태 요약 출력 |
| **inject-context** | 서브 에이전트 시작 (`SubagentStart`) | `rules.md`와 에이전트별 메모리(`memory/agents/{agent}.md`)를 서브 에이전트의 컨텍스트에 주입 |
| **protect-geas-state** | Write/Edit 후 (`PostToolUse`) | `.geas/**/*.json`에 타임스탬프 주입, intake 후 `seed.json` 수정 시 경고, `prohibited_paths` 확인 |
| **verify-task-status** | Write/Edit 후 (`PostToolUse`) | 작업이 "passed"로 표시될 때 5개 필수 증거 파일 존재 확인 |
| **restore-context** | 컨텍스트 압축 후 (`PostCompact`) | 컨텍스트 윈도우 압축 후 실행 상태와 규칙 재주입 |
| **track-cost** | 서브 에이전트 완료 후 (`SubagentStop`) | 에이전트, 태스크, 모델을 `costs.jsonl`에 기록하여 run-summary 비용 보고서에 활용 |
| **verify-pipeline** | 세션 종료 전 (`Stop`) | 모든 완료된 작업에 대한 필수 증거를 확인. **세션 종료를 차단할 수 있는 유일한 hook.** |

### 볼 수 있는 것

- **세션 시작 시 상태 요약**: 정상입니다. session-init hook이 현재 프로젝트 상태와 체크포인트 정보를 출력하고 있습니다.
- **에이전트가 프로젝트 규칙과 메모리를 받음**: 정상입니다. inject-context hook이 작업 시작 전에 서브 에이전트에 `rules.md`와 에이전트별 메모리 파일을 제공하고 있습니다.
- **조기 태스크 완료 경고**: verify-task-status hook이 필수 5개 증거 파일 없이 "passed"로 표시된 태스크를 감지했습니다. Compass가 누락된 단계를 실행하도록 합니다.
...
- **압축 후 컨텍스트 복원**: 정상입니다. restore-context hook이 컨텍스트 압축 이벤트 후 실행 상태를 재주입했습니다.
...
1. Compass가 일반적으로 누락된 증거를 감지하고 에이전트를 한 번 재시도합니다.
2. 에이전트가 오류를 만났는지 확인하세요 (도구 권한 거부, 파일 경로 문제 등).
3. `.geas/rules.md`가 존재하는지 확인하세요 -- 에이전트가 먼저 읽고, 누락되면 조용히 실패할 수 있습니다.
