...
`/geas:initiative`로 실행한다. Genesis, MVP Build, Polish, Evolution 네 단계를 순서대로 거친다. Contract Engine이 모든 단계를 통제한다 -- 근거 없이 에이전트가 완료를 자체 보고하는 건 불가능하다.
...
`/geas:sprint`로 실행한다. 기존 프로젝트에 범위가 정해진 기능 하나를 추가한다. Genesis를 건너뛴다. 기능 하나, 파이프라인 하나.
