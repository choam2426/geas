...
런타임이 hook 시스템을 제공하면 프로토콜 불변조건을 runtime이 직접 강제할 수 있다. 없으면 agent discipline과 사후 검사로 대체한다. 두 경로의 보장 강도는 같지 않다 — hook 런타임은 위반 시도 자체를 차단하는 tamper-evident 모델이고, degraded 런타임은 agent가 규약을 지킨다는 전제 위에서 사후에 검출만 가능한 best-effort 모델이다.
...
`geas validate`는 **schema 적합성**만 검증한다. "이 쓰기가 CLI를 경유했는가", "append-only 이력이 보존됐는가", "구현체 보조 파일이 몰래 바뀌지 않았는가"는 schema로 검출되지 않는다. 따라서 degraded 런타임의 보장은 다음과 같이 약해진다.

- Agent가 tool 권한 제한을 우회해 `.geas/`를 직접 수정한 경우 → 다음 `validate` 시점에 schema 위반이 되면 검출 가능, schema는 통과하지만 이력이 조작됐다면 검출 불가.
- Agent가 `geas` 명령을 우회해 append-only 로그의 중간 entry를 삭제한 뒤 새 entry를 만든 경우 → schema만 맞으면 validate는 통과.

Hook 런타임에서는 두 경우 모두 Edit/Write 시도 자체가 PreToolUse에서 거부되므로 tamper evidence가 남는다. Degraded 런타임은 이 간극을 agent 신뢰로 메꾼다는 걸 adapter 구현자와 운영자 모두 인지해야 한다.
...
2. **CLI-only writes** — `.geas/` 아래 어떤 쓰기도 CLI를 우회하지 않는다. 원칙은 모든 런타임에 동일하지만 강제 강도는 다르다 — hook 가능 런타임은 runtime-level 차단으로 tamper evidence까지 보장하고, 그렇지 않은 런타임은 agent 권한 제한과 사후 `validate`에 의존하는 best-effort 모델이다. §8.4 참조.
