...
| **Agent Teams** | 작업을 수행하는 전문 에이전트 집합. 기본 Geas-12 팀은 Nova(PM), Forge(아키텍트), Palette(디자이너), Pixel(프론트엔드), Circuit(백엔드), Sentinel(QA), Keeper(Git/릴리스), Shield(보안), Scroll(문서), Critic(반대 의견), Scrum(애자일 마스터), Pipeline(DevOps)으로 구성된다. | 파이프라인 스킬을 수정하면 교체 가능. 기본 Geas-12 팀은 하나의 구성이다. 대안 팀은 에이전트 이름을 참조하는 파이프라인 스킬을 수정해야 한다. Contract Engine의 데이터 흐름(TaskContract → Evidence → Gate)은 어떤 에이전트 구성에서도 동작한다. |
