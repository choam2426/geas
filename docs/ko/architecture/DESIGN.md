...
```yaml
---
name: mission-init                # 필수. 64자 이내, 소문자·숫자·하이픈만
...
|---|---|---|---|---|
| `/mission-init` | main_session | orchestrator | specifying phase | `mission create`, `mission design-set`, `task draft`, `task approve`, `mission-state update` |
...
Orchestrator는 agent 파일로 존재하지 않는다. 사용자와 직접 대화하는 메인 세션이 orchestrator 역할을 담당한다. mission-init, gate-run, task-close, phase-review 같은 메인 세션 skill은 이 세션 안에서 실행된다. 다른 slot은 필요할 때 orchestrator가 스폰한 별도 agent에서 실행된다.
