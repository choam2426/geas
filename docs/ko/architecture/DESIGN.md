...
- **memory system** 이 교훈을 포착하여 rules.md와 agent memory를 통해 미래 작업에 반영한다.
...
| **Contract Engine** | 불변 핵심. task 생명주기를 강제하는 12개 skill: intake, compilation, contracts, gates, verification, voting, memory, scheduling, setup, policy, reporting. | **불가** |
...
Contract Engine은 slot 이름(`implementer`, `quality_specialist`, `risk_specialist`, `operations_specialist`, `communication_specialist`)만 참조한다. Orchestrator가 런타임에 이를 구체적 agent로 해석한다. 동일한 12개 core skill이 소프트웨어, 연구, 기타 모든 도메인에서 작동하는 이유가 여기에 있다.
...
| **Evolving** | 교훈을 포착한다. 회고, rules.md 갱신, agent memory 갱신, 이월 백로그. | Evolution 산출물 기록, mission 요약 작성 |
...
agents/{agent}.md — agent별 메모 (역할 특화 교훈)
```

Memory는 두 파일로 구성된다. `rules.md`는 모든 프로젝트 지식을 담으며 모든 agent 컨텍스트에 주입된다. Agent memory 파일은 역할별 메모를 담으며 해당 agent에게만 주입된다.
...
3. **Memory 추출** — 교훈을 rules.md와 agent memory에 반영
...
> Memory 시스템: `protocol/07`. 진화 루프: `protocol/11`.
...
**작동 방식:** Setup skill이 초기화 시 프로젝트 관례를 감지하여 `.geas/rules.md`에 기록한다. Skill은 이 파일을 읽어 실행할 명령을 판단한다. 프로젝트 스택이 바뀌어도 skill 정의를 수정할 필요가 없다.
...
| Memory 시스템 | `07_MEMORY_SYSTEM_OVERVIEW` |
...
| 진화, 부채, gap 루프 | `11_EVOLUTION_DEBT_AND_GAP_LOOP` |
