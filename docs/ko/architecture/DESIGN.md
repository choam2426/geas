...
| Mode | 목적 | 코드 수정 |
|------|------|-----------|
| `discovery` | 방향, 범위, 구조, task inventory 형성 | 기본 금지 (spike 예외) |
| `delivery` | task backlog 실행, 구현/검증/통합 | 허용 |
| `decision` | 코드 없이 구조적 의사결정 | 금지 |

### Mission과 Phase

Initiative mission은 4개 phase를 거친다:
...
| Mode, mission, phase | `protocol/02_MODES_MISSIONS_AND_RUNTIME.md` |
