...
Payload를 받는 명령은 두 채널 중 하나로 payload를 받는다.

- **`--file <path>`** (크거나 prose가 많은 payload에 권장). CLI가 해당 경로의 파일 내용을 UTF-8 텍스트로 직접 읽는다. Shell이 payload 내용을 파싱하지 않으므로 아포스트로피·비-ASCII prose·개행·backtick이 섞여도 훼손되지 않는다. 경로는 어디든 가능하지만 `.geas/` 아래 직접 Write는 hook이 차단하므로 호출자는 `.geas/` 바깥(예: `/tmp/`, 프로젝트 로컬 scratch 파일)에 staging해야 한다.
- **stdin**. `--file`이 없으면 CLI가 stdin에서 읽는다. `cat file | geas cmd` 같이 믿을 수 있는 파이프로 흘려보낼 때만 안전하다. Bash heredoc으로 prose payload를 inline 주입하는 방식은 **피한다** — 인용부호·아포스트로피·비-ASCII 텍스트가 CLI에 도달하기 전에 shell 파싱에서 깨지는 경우가 많다.

정확히 한 채널만 payload를 공급해야 한다. `--file`과 stdin 둘 다 주어지면 `--file`이 우선 (stdin은 무시). 아무 채널도 없고 payload가 필요한 명령이면 `invalid_argument` (종료 코드 1). JSON payload 명령은 내용을 UTF-8 JSON으로 파싱하며 파싱 실패도 같은 에러 코드. 텍스트 payload 명령(`mission design-set`, `memory shared-set`, `memory agent-set`)은 BOM 제거와 말미 개행 정규화만 거쳐 그대로 기록한다.

이 계약은 "`--file` 또는 stdin으로 UTF-8 content를 받는다"까지만 규정한다. 어떤 쉘·런타임이 어떻게 content를 준비하는지는 호출자의 문제다. 아래 §4.4에 reference 구현체가 검증한 호출 패턴을 둔다.
...
**권장 — 파일에 staging 후 `--file` 전달**:

모든 쉘과 agent 환경에서 가장 안정적인 패턴. Payload 바이트가 shell 파서를 전혀 거치지 않는다.
