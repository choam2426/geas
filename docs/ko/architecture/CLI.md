...
모든 `.geas/` 경로는 프로젝트 루트 기준 상대. 루트 감지는 `.geas/config.json`을 통한다.
...
```
1. 현재 디렉토리부터 상위로 walk up하며 `.geas/config.json` 탐색
2. 첫 발견 config를 파싱
...
5. config 없이 `.geas/` 디렉토리만 있으면 구버전 호환: 해당 위치를 canonical로 간주
...
Delegate가 설정된 워크트리에서 쓰기 명령은 canonical 저장소의 `.geas/`에 실제 파일을 작성한다. Read 명령도 canonical을 본다.
