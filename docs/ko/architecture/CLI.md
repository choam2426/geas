...
| 명령 | 연산 | 대상 artifact |
|---|---|---|
| `geas memory shared-set` | set | `.geas/memory/shared.md` (stdin이 full markdown, atomic replace) |
| `geas memory agent-set --agent <type>` | set | `.geas/memory/agents/{type}.md` (stdin이 full markdown, atomic replace) |
...
JSON payload는 **stdin으로만** 받는다. 플래그로 주입하지 않는다. CLI는 stdin이 TTY가 아니면 UTF-8 JSON으로 해석한다. Payload가 필요한 명령인데 stdin이 비어 있거나 TTY면 입력 누락으로 실패한다 (`invalid_argument`, 종료 코드 1). JSON 파싱 실패도 같음.

이 계약은 "stdin으로 UTF-8 JSON을 받는다"까지만 규정한다. 어떤 쉘·런타임이 어떻게 payload를 공급하는지는 호출자의 문제다. 아래 §4.4에 reference 구현체가 검증한 shell 호출 패턴을 둔다.
...
계약은 위까지다. 이 서브섹션은 reference 구현체가 검증한 POSIX shell 호출 패턴을 예시로 보여 준다. fish·PowerShell·cmd 같은 다른 환경을 쓰는 adapter는 자기 환경에 맞는 equivalent 방식으로 stdin에 JSON을 공급하면 된다.
...
`<<'EOF'`의 quoted delimiter(`'EOF'`)가 shell 변수 확장과 backtick 평가를 차단한다. JSON 원문이 CLI에 그대로 전달된다.

⚠️ `<<EOF`(unquoted)은 피한다 — bash·zsh이 `$`·backtick을 치환해 JSON을 오염시킨다. bash·zsh에서는 항상 quoted delimiter를 쓴다.
...
| `implementing → reviewed` | `self-check.json` + `evidence/*.{slot}.json` | self-check 존재 + `contract.routing.required_reviewers`의 각 slot에 대해 해당 slot이 붙은 evidence 파일(어떤 concrete agent로든)이 존재하고 최소 1 entry |
| `reviewed → verified` | `gate-results.runs` | 마지막 run verdict = `pass` |
| `verified → passed` | `evidence/orchestrator.orchestrator.json` | 마지막 entry가 evidence_kind=closure + verdict=approved |
...
| `escalated → ready/implementing/reviewed` | `evidence/orchestrator.orchestrator.json` | 마지막 closure entry verdict=changes_requested |
| `* → cancelled` | — | 항상 허용 |
...
`geas memory shared-set`·`geas memory agent-set --agent <type>`는 stdin으로 받은 markdown 본문으로 대상 파일을 **전체 교체**한다 (atomic temp → fsync → rename). Schema 검증은 없다 — `.geas/memory/*.md`는 schema 검증 대상이 아닌 free-form markdown이다. Append 연산이 아니므로 호출자는 기존 파일을 먼저 읽어 변경 후 전체를 다시 제출한다.
