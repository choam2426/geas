...
| run state | `.geas/state/run-state.json` | `docs/schemas/run-state.schema.json` | doc 02 |
...
| rules update | `.geas/missions/{mission_id}/consolidation/rules-update.json` | `docs/schemas/rules-update.schema.json` | doc 07 |
| debt register | `.geas/missions/{mission_id}/consolidation/debt-register.json` | `docs/schemas/debt-register.schema.json` | doc 08 |
| gap assessment | `.geas/missions/{mission_id}/consolidation/gap-assessment.json` | `docs/schemas/gap-assessment.schema.json` | doc 08 |
...
| mission design | 접근 방향, 핵심 결정, 비목표, 검증 방향 기술 |
| decision record | 구조화된 결정과 그 근거 기록 |
| 초기 task compilation 출력 | mission spec과 mission design에서 생성된 task contract |
...
| `verdict` | task closure decision | verdict (`approved`/`changes_requested`/`escalated`), rationale |
...
| `decision` | `evidence-decision.schema.json` | `approved`, `changes_requested`, `escalated` | Orchestrator |

모든 evidence_kind는 선택 필드 `memory_suggestions[]`를 공통으로 둘 수 있다.
역할은 파일명이 아니라 내부 필드로 식별한다.

Orchestrator의 task closure decision은 `decision` evidence로 남기고, task 수준 종결 상태는 `record.json`의 `verdict` 섹션에 함께 기록한다. 미션 final verdict는 task artifact가 아니라 mission 종결 판단이다.
...
| artifact | 파일 | 설명 |
|---|---|---|
| `run-state.json` | `state/run-state.json` | phase, 활성 task, checkpoint를 포함한 mission 수준 런타임 상태 |
...
Consolidating phase에서 생성되는 artifact로, 팀이 성과를 돌아보고 프로세스를 갱신하는 산출물이다.

| artifact | 파일 | 설명 |
|---|---|---|
| retrospective | `record.json` retrospective 섹션 | 무엇이 잘 되었고, 무엇이 실패했고, 무엇을 바꿀지에 대한 구조화된 회고 |
| rules update | `consolidation/rules-update.json` | 프로젝트 규칙에 대한 제안 또는 적용된 변경 |
| debt register | `consolidation/debt-register.json` | 추적 중인 기술적 또는 프로세스 부채 항목 |
| gap assessment | `consolidation/gap-assessment.json` | 프로토콜 요구사항과 실제 관행 간 식별된 격차 |
...
.geas/
  ...
  missions/{mission_id}/
    ...
    polishing/                        # polishing phase evidence
      evidence/{agent}.json
    consolidation/                    # gap-assessment, debt-register, rules-update
      evidence/{agent}.json           # consolidating phase evidence
    tasks/{task_id}/
...
| runtime-summary drift | 요약이 canonical artifact로 뒷받침되지 않는 상태나 verdict를 주장 | 세션 요약은 task 통과를 말하나 task closure decision artifact가 없음 |
...
| **파싱 가능하지만 의미론적으로 불가능** | 다운스트림 진행을 차단하고, JSON 파싱이 되더라도 의미론적 무효로 처리 (예: task closure decision이 `approved`인데 task가 `verified`가 아님, gate result가 `pass`인데 필수 검사가 없음) |
