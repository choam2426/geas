...
| shared memory 변경 자동 승인 | memory update는 근거와 승인 절차가 필요하다 |
...
- task state의 `write_phase`가 `pending`인데 committed처럼 복원하려는 경우
- mission state가 주장하는 현재 task와 실제 artifact가 모순되는 경우
- 신뢰할 artifact 자체가 흐트러진 상태에서 escalate 없이 자동 전이를 시도하는 경우
...
| learning | memory update 채택 수, 잘못된 재사용 rollback 수 |
