...
Debt register와 gap assessment는 evidence만 옮겨 적어 만드는 문서가 아니다. 미션 기준선, task 계약, 실제 산출물, evidence를 함께 보고 정리해야 한다.

대신 각 evidence는 후속 정리에 필요한 신호를 올릴 수 있다.

| 필드 | 의미 |
|---|---|
| `debt_candidates[]` | 나중에 debt register로 올릴 수 있는 타협 후보 |
| `gap_signals[]` | 약속한 것과 실제 전달한 것 사이의 차이를 시사하는 신호 |

이 두 필드는 입력 신호일 뿐이며, 그 자체로 debt 등록이나 gap 판단이 확정되지는 않는다. 최종 정리는 phase review, gap assessment, debt register에서 이뤄진다.
...
| mission final verdict note | Decision Maker가 조건부 수용 |
...
`gap_signals[]`는 gap assessment의 입력이 될 수 있지만, assessment는 mission 기준선과 실제 전달 범위를 함께 보고 작성해야 한다.
