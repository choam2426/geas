...
- `run-state.json` (`checkpoint` 필드로 마지막 진행 상황 확인)
...
| closure packet 저장됨, task closure decision 미발행 | closure packet은 조립되었으나 Orchestrator가 아직 종결 판단을 내리지 않음 |
...
| task closure decision 주장되었으나 packet 누락 | verdict가 기록되었으나 이를 뒷받침하는 closure packet이 없음 |
...
| passed 주장, task closure decision 누락 | invalid | hard block |
...
### Scenario 2 -- 로컬에서 integration 완료, task closure decision 미생성

Integration 결과가 존재하고 유효하면:

- post-integration, pre-closure-decision safe boundary에서 복구한다
