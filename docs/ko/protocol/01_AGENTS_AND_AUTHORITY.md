...
> GEAS가 어떤 slot 계열을 두고, 누가 어떤 경계 안에서 설계, 조율, 승인, 이의를 담당하는지 정의한다. Task evidence와 mission/runtime artifact는 각각 doc 03, doc 02, doc 05, doc 08이 owner다.

## 목적

GEAS는 먼저 권한 경계를 고정한다. 누가 무엇을 결정할 수 있는지 흐려지면 검증, 승인, 회고가 한 agent에 섞여 판단 근거가 약해진다. 이 문서는 slot family와 authority boundary를 정의한다.
...
- closure packet에 근거해 task closure decision을 남긴다.
...
- mission spec을 승인하거나 보류한다.
- mission design을 승인하거나 보류한다.
- 초기 task contract 집합을 승인하거나 보류한다.
...
- 각 task가 닫힐 수 있는 단위인지, acceptance criteria와 rubric이 충분한지 점검한다.
