...
Deliberation은 미션 수준에서 명시적 다자 판단이 필요할 때 남기는 구조화된 심의 기록이다. 정확한 구조는 `deliberation.schema.json`이 관리하며, 같은 schema가 task-level deliberation에도 쓰인다. level은 artifact 내부의 `level` 필드로 구분하며, task-level일 때는 `task_id`도 함께 기록한다.
