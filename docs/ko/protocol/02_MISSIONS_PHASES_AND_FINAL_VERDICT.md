...
Mission은 세 artifact로 정의된다.

- **Mission spec** — 미션이 무엇을 만족해야 하는지 (시작 계약)
- **Mission design** — 어떻게 달성할지 (계획)
- **Mission verdict** — 미션이 어떻게 종결되었는지 (종결 판단)

이와 함께 **operating mode**가 미션의 판단 깊이를 정한다 (spec의 `mode` 필드).

Spec과 design은 specifying phase에서, verdict는 consolidating phase에서 확정된다.
...
**이 phase의 활동** (승인 순서는 고정):

1. Orchestrator가 사용자와의 대화·토론으로 mission spec 초안 작성
2. 사용자가 mission spec 승인
3. Design Authority가 mission design 작성
4. Mode별 선행 절차:
   - `lightweight`: 없음
   - `standard`: Decision Maker가 사용자 승인 전 리뷰
   - `full_depth`: Decision Maker·Challenger·specialist 1명의 deliberation (사용자 승인 전 필수)
5. 사용자가 mission design 승인
6. Design Authority가 초기 task contract 집합 작성
7. Mode별 선행 절차 (4와 동일)
8. 사용자가 초기 task contract 집합 승인
9. Orchestrator가 실행 가능성·reviewer 구성 가능성 점검 후 phase gate 판정

**Slot별 책임**:

| Slot | 맡는 일 |
|---|---|
| Orchestrator | 사용자 대화 진행, mission spec 작성, 실행 가능성·reviewer 구성 가능성 점검 |
| Design Authority | mission design 작성, 초기 task contract 집합 작성 |
| Decision Maker | `standard`·`full_depth`에서 사용자 승인 전 리뷰 또는 deliberation voter |
| Challenger | `full_depth` deliberation voter (필수) |
| Specialist | `full_depth` deliberation voter 1명 (필수) |
| User | mission spec·mission design·초기 task 집합 최종 승인 |

**최종 승인 주체는 언제나 사용자**다. Mode는 사용자 승인 전 선행 절차의 깊이만 바꾼다.
...
**이 phase의 활동** (각 task가 반복):

1. Scheduler가 `ready` task를 dispatch (의존 `passed` + baseline 유효 확인)
2. Implementer가 implementation contract 작성, 구현, self-check 기록
3. Required reviewer가 review evidence 제출, verifier가 verification evidence 제출
4. Orchestrator가 evidence gate run
5. Orchestrator가 task closure decision (closure evidence)
6. 필요 시 task-level deliberation: Orchestrator가 소집
7. 필요 시 scope 내 추가 task contract 생성: Decision Maker 승인
8. 이번 phase에서 끝내기로 한 task가 모두 `passed` 또는 carry-forward로 정리되면 phase gate 판정

**Slot별 책임**:

| Slot | 맡는 일 |
|---|---|
| Orchestrator | task 순서·상태 관리, evidence gate 실행, task closure decision, 필요 시 task-level deliberation 소집 |
| Specialist | task별 implementer / reviewer / verifier 역할 수행 (상세는 doc 03) |
| Challenger | 필요 시 task-level challenge |
| Decision Maker | scope 내 추가 task contract 승인, task-level deliberation voter |
...
1. Orchestrator가 통합 상태 점검
2. Design Authority와 관련 specialist가 남은 구조적 부족분을 식별
3. 각 부족분을 분류: 새 task / debt / gap / 조치 없음
4. 새 task가 필요하면 contract 작성·승인 후 `building`으로 역행
5. 새 task가 필요 없으면 phase gate 판정 후 `consolidating`으로 전이

**Slot별 책임**:

| Slot | 맡는 일 |
|---|---|
| Orchestrator | 통합 상태 정리, 부족분 처리 방법 판정 |
| Design Authority | 구조적 부족분 식별, 필요 시 신규 task contract 작성 |
| Specialist | 자기 lens에서 구조적 부족분 식별 보조 |
| Decision Maker | 신규 task contract 승인 (scope 내) |
...
1. Orchestrator가 모든 evidence·closure·deliberation을 훑어 `memory_suggestions`·`debt_candidates`·`gap_signals` 집계
2. Design Authority가 scope closure 점검 후 gap 작성
3. Orchestrator가 debt·memory update 확정
4. Decision Maker가 mission의 모든 artifact를 종합해 mission final verdict
5. Phase gate 판정 → mission phase `complete`

**Slot별 책임**:

| Slot | 맡는 일 |
|---|---|
| Orchestrator | memory·debt 확정, carry-forward 정리, 종결 근거 조립 |
| Design Authority | scope closure 점검, gap 작성 |
| Decision Maker | mission final verdict |
