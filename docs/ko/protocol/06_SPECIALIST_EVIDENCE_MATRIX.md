...
specialist review는 closure를 돕는 것에서 끝나지 않는다. 아래 중 하나라도 있으면 `process_lead`가 evolution pipeline 입력으로 넘긴다.
- reusable lesson
...
하나의 specialist review에 여러 종류의 memory candidate가 포함된 경우, `process_lead`는 아래 우선순위로 처리한다:
