...
도메인 프로필은 범용 슬롯 기대치를 구체적인 검토 대상으로 세분화한다. 아래는 대표적인 예시이며 완전한 목록이 아니다.

#### 소프트웨어 개발 프로필

| 슬롯 | 구체 타입 | 도메인별 검토 초점 |
|---|---|---|
| Implementer | `software-engineer` | 변경된 UI 경로, 인터랙션 상태, 반응형 동작, 접근성, API 계약, 데이터 흐름, 마이그레이션 안전, 오류 처리, 멱등성 |
| Quality Specialist | `qa-engineer` | 기준 대비 테스트 커버리지, 부정 경로, 시연 검증 |
| Risk Specialist | `security-engineer` | 인증/인가 경계, 비밀 관리, 인젝션 표면, 남용 경로 |
| Operations Specialist | `platform-engineer` | CI 안정성, 배포 영향, 설정 드리프트, 출처 관리 |
| Communication Specialist | `technical-writer` | 문서 완전성, 마이그레이션 안내, 운영자 주의사항 |

#### 연구 프로필

| 슬롯 | 구체 타입 | 도메인별 검토 초점 |
|---|---|---|
| Implementer | `research-analyst` | 방법론 타당성, 데이터 수집 유효성, 분석 재현성 |
| Quality Specialist | `methodology-reviewer` | 통계적 엄밀성, 표본 적정성, 발견 재현 가능성 |
| Risk Specialist | `research-integrity-reviewer` | 윤리 준수, 데이터 프라이버시, 편향 평가 |
| Operations Specialist | `research-engineer` | 데이터 파이프라인 안정성, 컴퓨팅 재현성, 환경 일관성 |
| Communication Specialist | `research-writer` | 논문 명확성, 인용 정확성, 독자 수준에 맞는 발표 |

#### 콘텐츠 제작 프로필

| 슬롯 | 구체 타입 | 도메인별 검토 초점 |
|---|---|---|
| Implementer | `content-writer` | 사실 정확성, 톤 일관성, 출처 명시 |
| Quality Specialist | `fact-checker` | 주장 검증, 출처 신뢰도, 교차 참조 일관성 |
| Risk Specialist | `legal-reviewer` | 저작권 준수, 책임 노출, 규정 적합성 |
| Operations Specialist | `publishing-engineer` | CMS 통합, 포맷 변환, 배포 준비 |
| Communication Specialist | `editor` | 독자 적합성, 스타일 일관성, 명확성 |
...
에이전트는 재사용 가능한 지식을 발견하면 산출물에 memory_suggestions를 포함해야 한다. Orchestrator는 회고 과정에서 이를 수확하고, 확인된 제안을 `.geas/memory/agents/{agent_type}.md`에 반영한다.
