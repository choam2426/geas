...
| **Decision Maker** | `decision-maker` | 미션 설계 승인, 미션 final verdict, 트레이드오프 판단 | X | O | X | O | O |
...
- task 라우팅 배정
- 동시성, 잠금, 재검증, 복구 관리
- task closure decision에 필요한 근거 정리와 판단
- 단계 경계에서 학습 artifact와 교훈을 정리해 메모리에 반영

금지:

- 미션 final verdict를 내릴 수 없다
- 구현 계약을 승인할 수 없다
- 필수 리뷰를 임의로 줄이거나 생략할 수 없다
- 빠진 evidence를 직접 작성할 수 없다
- 해소되지 않은 갈등이나 반대 의견을 종결 근거에서 숨길 수 없다
...
- 불완전한 미션 종결 근거로 final verdict를 내릴 수 없다
- 기록된 차단 의견을 근거 없이 지울 수 없다
- 필수 리뷰 없이 미션 final verdict를 내릴 수 없다
...
참여와 판단은 이름이 아니라 evidence로 남아야 한다. 해당 책임을 실제로 수행하고 그 결과를 기록했을 때만 참여나 판단으로 인정한다. 이 절의 evidence는 task root 아래에 저장되는 task-scoped artifact를 가리킨다.
...
| `decision` | task closure decision과 그 근거를 남긴다 | 있음 |

Evidence는 `slot → evidence_kind → verdict` 순서로 읽는다. `slot`은 책임을, `evidence_kind`는 구조를, `verdict`는 그 구조 안에서 남긴 판단을 뜻한다.

모든 evidence_kind는 선택 필드 `memory_suggestions[]`, `debt_candidates[]`, `gap_signals[]`를 공통으로 둘 수 있다. `memory_suggestions[]`는 미래 행동을 바꿀 가치가 있는 교훈을, `debt_candidates[]`와 `gap_signals[]`는 후속 debt/gap 정리에 필요한 신호를 담는다. 두 필드의 정리 규칙은 doc 08을 따른다.

Evidence 참조가 없는 검토나 종결 판단은 신뢰도가 낮은 입력으로 취급해야 하며 높은 엄격도 수준에서는 단독으로 종결 근거가 될 수 없다.
...
| verdict | 의미 |
|---|---|
| `approved` | 현재 task를 닫고 `passed`로 넘길 수 있다고 판단 |
...
| Orchestrator | `decision` | task closure decision과 그 근거를 남긴다 |

Task 수준 종결 상태는 `record.json`의 `verdict` 섹션에 함께 남긴다. 미션 final verdict는 task evidence 패밀리가 아니라 mission 종결 단계의 판단으로 다룬다.
...
필수 리뷰어 간 갈등 해소, vote round, task closure decision, mission final verdict 절차는 doc 05를 따른다.
...
- 미션 final verdict는 명시적 Decision Maker 역할 전환 아래에서 내려야 한다
...
- 누가 리뷰했는지
- 누가 반론을 제기했는지
- 누가 task closure decision을 내렸는지
- 누가 mission final verdict를 내렸는지
