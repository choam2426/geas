...
모든 `passed` task 뒤에는 `process_lead`가 `retrospective.json`을 작성한다.
...
### rule update 승인
아래 중 하나 필요:
- `process_lead` + domain authority 승인
...
1. `process_lead`가 해당 domain authority와 함께 검토한다.
...
1. `process_lead`는 해당 rule의 memory 의존성을 검토한다.
