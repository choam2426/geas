...
| 금지 행위 | 이유 |
|---|---|
| product verdict 발행 | verdict는 자동화가 아닌 agent 판단을 요구 |
...
| 전환 | 필수 Artifact |
|---|---|
| drafted → ready | `contract.json`에 필수 필드 존재 (task_kind, risk_level, gate_profile, vote_round_policy, base_snapshot, rubric.dimensions[]) |
...
| 전환 | Gate 기준 |
|---|---|
| specifying → building | `spec.json` 존재, `design-brief.json` 승인됨, tasks/에 task ≥1개 |
| building → polishing | 모든 task가 passed 또는 cancelled, `evolution/gap-assessment-building.json` 존재 |
| polishing → evolving | `evolution/gap-assessment-polishing.json` 존재, `evolution/debt-register.json` 존재 시 critical open 항목 없음, blocked/escalated task 없음 |
| evolving → complete | `evolution/gap-assessment-evolving.json` 및 `mission-summary.md` 존재, `evolution/debt-register.json` 존재 시 critical open 항목 없음 |

미충족 기준이 있으면 exit code 1과 함께 `unmet_criteria` 배열을 반환한다.

> **참고:** Phase 참조 문서(`specifying.md`, `building.md`, `polishing.md`, `evolving.md`)는 각 phase의 권장 절차 전체를 기술한다. Phase 가드는 기계적 전제 조건, 즉 artifact 존재와 상태 조건만 강제한다. 가드는 전체 절차의 부분 집합이지 완전한 대체가 아니다. 에이전트는 전체 참조 절차를 따르되, 가드는 최소 gate 기준을 강제한다.
