...
Baseline은 task가 출발할 때 신뢰한 공유 기준선이다. Task contract의 `base_snapshot`은 그 기준선을 붙드는 필드다.

### baseline 규칙

- `ready`에서 `implementing`으로 들어가기 전, baseline이 여전히 유효한지 확인해야 한다.
...
Workspace는 task가 실제 변경을 쌓는 작업 표면이다. 프로토콜은 구현체의 구체 방식은 강제하지 않지만, 한 task의 변경과 다른 task의 변경을 구분할 수 있을 만큼은 격리되어야 한다.
...
- 하나의 task가 다른 task의 미완료 결과를 전제로 하지 않는다.

### 병렬 실행을 피해야 하는 경우

- 같은 인터페이스나 같은 자원을 동시에 바꾸는 경우
- integration 순서가 결과 품질에 직접 영향을 주는 경우
- 어느 task의 검증 실패가 다른 task의 판단 근거를 무너뜨리는 경우
...
- Runtime이 `scheduler_state` 같은 실행 상태를 노출할 수는 있지만, 그것이 병렬 안전성을 스스로 보장하지는 않는다.
