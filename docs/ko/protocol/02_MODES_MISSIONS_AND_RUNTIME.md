...
런타임 phase는 각 미션 phase 내에서 순환 패턴을 따른다:

```text
bootstrap (세션당 1회)
→ planning
→ scheduling → executing → integrating → verifying (phase 종료 조건 충족까지 반복)
→ learning
→ idle
```

Phase를 건너뛸 수 없지만, 단일 task 세션에서는 일부 phase가 순간적으로 통과될 수 있다.
...
- 미해결 recovery 조건이 없음: 이전 세션의 recovery가 완료되었거나 recovery 대상이 없고, baseline이 확인 가능한 상태이며, 이전 미션의 orphan task가 남아있지 않음
...
3. 반복 실패 시도는 재범위화 또는 escalation을 유발해야 한다.
4. 실패 이력은 phase-review 기록에 가시적으로 남아야 한다.

## 긴급 예외

프로젝트는 hotfix나 인시던트 대응을 위해 긴급 경로를 사용할 수 있다. 긴급 경로는 강화 메커니즘을 유예하되 무결성 메커니즘은 유지하는 방식으로 작동한다.

프로토콜은 두 종류의 메커니즘을 구분한다:

- **무결성 메커니즘** — 결과가 실제로 동작하는지 확인 (evidence gate, final verdict, artifact 생산)
- **강화 메커니즘** — 결과를 더 안전하고 견고하게 만듦 (polishing phase, vote round, critical review challenge)

긴급 경로 규칙:

- 강화 메커니즘은 유예하고 부채로 등록할 수 있다
- 무결성 메커니즘은 유예할 수 없다: task artifact, evidence gate, final verdict, evolving phase는 여전히 필수
- 예외 사유를 반드시 기록한다
- 유예된 항목은 모두 명시적 후속 조치가 포함된 부채로 등록한다
- 미션은 진정한 종료 전에 반드시 evolving을 거친다

긴급 예외는 허점이 아니라 시간 압박 하에서도 추적성을 보존하는 통제된 성능 저하 경로다.
