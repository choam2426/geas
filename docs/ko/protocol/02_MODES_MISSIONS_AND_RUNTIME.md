...
Phase 전이마다 생산되는 evidence 묶음이다. 이 artifact 체인이 미션 의도에서 전달 결과까지의 추적성을 만든다. Phase review artifact 없이 전이를 시도하면 프로토콜 위반이다.

### 전이별 필수 artifact

| 전이 | 필요한 것 |
|---|---|
| `specifying` → `building` | `phase-review.json`: 미션 확정, task 준비 완료, design-brief 승인(필요 시) 확인 |
| `building` → `polishing` | `phase-review.json`, `gap-assessment.json` |
| `polishing` → `evolving` | `phase-review.json`, `gap-assessment.json`, 현재 debt 현황 |
| `evolving` → close | `phase-review.json`, `gap-assessment.json`, retrospective 묶음, debt snapshot, mission summary |

### 추가 권장 artifact

미션의 맥락과 결정 이력을 보존하는 데 유용하다. Assurance profile이 높을수록 포함을 강하게 권장한다.

| artifact | 담는 것 |
|---|---|
| `debt-register.json` | 미션 동안 쌓인 기술·프로세스 부채 |
| `rules-update.json` | 미션에서 도출된 규칙 변경 |
| `decision-record.json` | 주요 범위 변경이나 갈등 해소의 근거 |
| `run-summary.md` | 미션 실행을 사람이 읽을 수 있는 형태로 요약 |
