...
미션은 개방형 요청을 실행 가능한 프로토콜 객체로 변환한다. 미션 계층은 구현을 시작하기 전에 다음 질문 전부에 답할 수 있도록 존재한다:
...
Specifying phase는 원시 요청을 구조화되고 실행 가능한 미션 정의로 변환한다. 팀이 구현을 안전하게 시작할 수 있을 만큼 충분한 명확성을 확보하면 종료된다.

목적:

- 요청을 미션 언어로 정규화
- `scope_in`의 첫 번째 이해를 고정
- 주요 설계 결정을 해소하거나 기록
- 초기 task 컴파일
- 미션에 필요한 엄격도 결정
...
- mission spec
- `standard` 또는 `full_depth` 작업에서 설계 결정이 중요할 때 승인된 design brief
- 초기 task set
- 초기 리스크 / 제약 요약
- 환경 준비 판단
...
- 미션 정의가 실행에 충분히 안정적
- task 목록이 실행 가능
- blocking 모호성이 해소되었거나 명시적 탐색 task로 전환됨
- 필요 시 design brief 승인이 기록됨

### 2) `building`

Building phase는 미션의 핵심 가치 경로를 구현한다. Task가 ready에서 passed까지 전체 생명주기를 거치는 주요 phase다.
...
- 모든 MVP-critical task가 `passed`
- 미해결 blocking 충돌 없음
- critical debt가 부재하거나 공식적으로 escalate됨
- scope creep가 숨겨지지 않고 가시적이며 평가됨

### 3) `polishing`

Polishing phase는 릴리스, 채택, 내부 신뢰를 위해 결과물을 강화한다. 품질 보증, 사용성, 문서, 운영, 보안, 유지보수성 등 모든 관련 품질 차원에 걸쳐 specialist-slot 리뷰를 적용한다.

목적:

- 릴리스, 채택, 내부 신뢰를 위한 결과 강화
- 모든 관련 품질 차원에 걸친 specialist-slot 리뷰 수행
- dead artifact, boilerplate, drift, 숨겨진 지름길 같은 엔트로피 식별

종료 gate 기대:

- high 및 critical debt가 분류됨
- 출시 표면에 대한 필수 specialist 리뷰 완료
- 알려진 리스크에 기록된 근거 존재
- 문서 및 운영 준비가 명백히 낡지 않음

### 4) `evolving`

Evolving phase는 학습된 내용을 포착하고 시스템을 미래 작업에 대비시킨다. 교훈 추출, 부채 통합, memory 시스템 피드를 통해 미션이 기억상실로 끝나는 것을 방지한다.
...
- 교훈, memory, 규칙 추출
- 부채 통합
- 기억상실로 끝나지 않고 미래 작업 준비

종료 gate 기대:

- gap assessment 존재
- retrospective 묶음 존재
- rules 및 memory 조치가 기록됨
- debt snapshot 캡처됨
- mission summary 존재
...
미션은 비자명 변경에 대해 종료 전 반드시 `evolving`에 진입해야 한다. 프로젝트는 작은 task에서 evolving phase를 압축할 수 있지만, 교훈 / 부채 / gap 표면은 반드시 보존해야 한다.

## Phase Review Artifact

Phase review artifact는 각 phase 전이 시 생산되는 evidence다. 미션 의도와 전달된 결과를 연결하는 traceability 체인을 형성한다.

### 필수 artifact

| 전이 | 최소 필수 artifact set |
|---|---|
| `specifying -> building` | 미션 고정, task 준비, 필요 시 design-brief 승인을 확인하는 `phase-review.json` |
| `building -> polishing` | `phase-review.json`, `gap-assessment.json` |
| `polishing -> evolving` | `phase-review.json`, `gap-assessment.json`, 현재 debt 뷰 |
| `evolving -> close` | `phase-review.json`, `gap-assessment.json`, retrospective 묶음, debt snapshot, mission summary |

### 권장 artifact

| artifact | 용도 |
|---|---|
| `debt-register.json` | 축적된 기술 및 프로세스 부채 |
| `rules-update.json` | 미션 중 생산된 규칙 변경 |
| `decision-record.json` | 주요 재범위화 또는 충돌 해소의 근거 |
| `run-summary.md` | 미션 실행의 사람이 읽을 수 있는 요약 |
