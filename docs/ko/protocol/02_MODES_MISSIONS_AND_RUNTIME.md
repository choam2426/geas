...
| `mode` | 미션 운영 모드: `lightweight`, `standard`, `full_depth`, `recovery_first`, `audit_only` |
...
- 긴급 상황에서는 별도의 긴급 예외를 만들지 않고 `lightweight` 모드를 사용해야 한다. Lightweight 모드는 의례를 최소화하면서도 모든 기본 불변식을 보존한다.
