...
- `00_PROTOCOL_FOUNDATIONS.md` — 핵심 목표, 불변 규칙, 준수 단위, 우선순위, 위협 모델
- `01_AGENT_TYPES_AND_AUTHORITY.md` — 역할 분류, 직무 분리, 리뷰어 라우팅, 갈등 해소
- `02_MODES_MISSIONS_AND_RUNTIME.md` — 미션 모델, mission mode, 4-phase, 런타임 phase, phase gate
- `03_TASK_MODEL_AND_LIFECYCLE.md` — task 메타데이터, 계약 품질, 생명주기 상태, 상태 복원, retry·iterate 규칙
- `04_BASELINE_WORKSPACE_SCHEDULER_AND_PARALLELISM.md` — baseline 최신성, 재검증, workspace 생명주기, lock·스케줄러
- `05_GATE_VOTE_AND_FINAL_VERDICT.md` — gate, 루브릭 채점, vote round, closure packet, final verdict
- `06_SPECIALIST_EVIDENCE_MATRIX.md` — specialist 슬롯별 최소 evidence 의무, 포함 규칙
- `07_MEMORY_SYSTEM_OVERVIEW.md` — memory 목적, 수용 기준, confidence, 행동 표면
- `08_EVOLVING_MEMORY_LIFECYCLE.md` — candidate → canonical 생명주기, decay, supersession, harmful reuse rollback
...
읽기: `00`, `05`, `09`, `10`, `12`, `14`

### 학습 시스템을 이해하려는 경우

읽기: `07`, `08`, `09`, `14`

### 문제가 발생한 세션을 추적하려는 경우

읽기: `03`, `04`, `05`, `10`, `11`, `12`

## 3. 호환성 참고

v2 프로토콜은 artifact 중심 모델을 유지하면서 다음을 추가한다:
