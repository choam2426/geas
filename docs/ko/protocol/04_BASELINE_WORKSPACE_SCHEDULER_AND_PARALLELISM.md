...
- **표면적 충돌**: 같은 영역을 직접 수정해서 물리적으로 충돌
- **semantic drift**: 영역은 다르지만 가정이나 동작이 바뀌어 task의 전제가 깨짐 (예: API 응답 형식이 바뀌었는데 해당 API를 호출하는 task가 이를 모름)
...
- `affected_surfaces`가 겹치지 않음
- lock 충돌이 없음
- 한 task의 산출물이 다른 task의 입력이 아님
- Design Authority가 독립성을 확인함
...
Preemption checkpoint에 포함해야 하는 것:
