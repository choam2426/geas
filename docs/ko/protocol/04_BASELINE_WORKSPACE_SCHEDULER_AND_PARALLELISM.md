...
Baseline 참조는 문서 버전, 데이터셋 체크포인트, 버전 관리 커밋(예: Git), 또는 도메인에 적합한 다른 재현 가능한 상태 참조가 될 수 있다.
...
Workspace는 task 실행을 위한 격리된 실행 환경이다 (doc 00 용어집 참조). 샌드박스 환경, 전용 작업 디렉토리, 버전 관리 worktree(예: Git), 또는 도메인에 적합한 다른 격리 메커니즘이 될 수 있다.
...
런타임 상태 디렉토리(예: `.geas/`)는 baseline 추적 대상이 아니므로 workspace에 포함되지 않는다. 격리된 작업 디렉토리나 버전 관리 worktree 등으로 workspace를 생성해도 런타임 상태 디렉토리는 복제되지 않는다. workspace 안에서 상대경로로 런타임 상태에 접근하면 존재하지 않는 경로를 참조하게 된다.
