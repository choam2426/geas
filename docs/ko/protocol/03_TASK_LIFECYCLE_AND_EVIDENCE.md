...
| `reviewed` | required reviewer의 review가 제출되었음 |
| `verified` | evidence gate가 검증 통과 또는 차단 여부를 판정했음 |
| `passed` | task closure decision이 `approved`로 끝나 task가 종결됨 |
...
| `escalated` | task 수준에서 종결하지 못해 상위 판단으로 넘김 |
...
| `ready` | 승인된 contract로 실행 대기. baseline 유효성과 의존 task 상태를 재확인한다 | — |
| `implementing` | Implementer가 `implementation-contract.json`을 작성한 뒤 구현하고 self-check를 남긴다. Required reviewer도 이 상태 동안 각자의 evidence를 제출한다 | `implementation-contract.json`, `self-check.json`, task에 해당하는 `evidence/{agent}.json` 집합 |
| `reviewed` | 필수 evidence가 모두 제출된 상태. evidence gate가 pass/fail/block/error를 판정한다 | `gate-results/{gate_run_id}.json` |
| `verified` | gate 결과를 바탕으로 Orchestrator가 task closure decision을 내린다 | closure `evidence/orchestrator.json` |
| `passed` | 종결 상태. 이후 consolidating phase가 이 task의 signal을 읽는다 | — |
