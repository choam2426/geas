...
필드 수준의 schema 명세는 doc 09, 집행과 메트릭은 doc 10을 참고한다.
...
| `memory_engine` | rules.md 관리, agent memory 주입, memory 후보 추출 로직 |
| `recovery_engine` | 체크포인트, 안전 경계 탐지, 상태 복원 / 재개 로직 |

노출하는 모든 단위가 이 문서의 필수 규칙을 충족하지 않으면 "Geas 완전 준수"를 주장할 수 없다.
...
- rules.md의 메모리 항목이 현재 task의 수용 기준을 덮어쓸 수 없다.
...
- 회고
- 규칙 갱신 루프
- rules.md 및 agent 노트에 대한 메모리 업데이트
- 기술 부채 등록과 해소
- gap 평가
- 미션 간 이월
...
불필요한 자율성보다는 직접적이고 조합 가능한 워크플로가 낫다. 에이전트 자율성을 추가하려면 측정 가능한 이점, 리스크 감소, 또는 task 자체가 단순화할 수 없을 만큼 복잡해야 한다.
...
9. 낡은 메모리 항목은 모순되는 현행 지침과 공존시키지 말고 삭제하거나 갱신해야 한다.
...
| stale·적대적 입력에 의한 컨텍스트 오염 | 선택적 컨텍스트 주입, rules.md 관리, agent memory 검토 |
| 도구 오남용이나 과도한 위임 | 도구 계약, 권한 경계, 미션 모드 엄격도 |
| 중단 뒤 복구 환각 | 2단계 체크포인트, 안전 경계, 복구 테이블 |
| 메모리 비대화·유해 재사용 | rules.md 관리, 잘못된 항목 직접 삭제, agent memory 검토 |
...
표준이 발전하더라도 검증기의 예측 가능성을 유지하기 위한 규칙이다.
...
- **Ledger** — `.geas/state/events.jsonl`에 주요 프로토콜 행동을 기록하는 추가 전용 이벤트 로그
- **Record** — task별 단일 실행 기록 파일(`record.json`)로, 모든 파이프라인 단계 산출물을 누적한다
- **Packet** — 특정 task나 판단을 위해 에이전트에 전달하는 컨텍스트 묶음
- **Context Engineering** — 에이전트 상호작용마다 최소한의 고신호 토큰 집합을 골라내는 실천 (doc 07)
...
Geas는 멀티 에이전트 소프트웨어 개발을 위해 처음 설계되었지만, 핵심인 통제된 의사결정, evidence 기반 완료, 체크포인트 복구, 학습 피드백 루프는 특정 도메인에 묶이지 않는다.
...
Geas는 취향이 아니다. AI가 돕는 구조화 작업을 위한 운영 헌법이다. 가장 빠른 길이 느려지더라도 evidence 무결성, 역할 명확성, 보수적 복구, 학습 가능한 피드백 루프는 반드시 지킨다.
