...
task가 무엇을 만들거나 수행하는지를 나타낸다. `task_kind`는 task를 완전히 설명하는 분류가 아니라, 상위 수준의 라우팅 힌트와 기록 기준을 제공하는 최소 분류다. 정규 enum은 `docs/schemas/task-contract.schema.json`에 정의되어 있다.

| task_kind | 의미 | 기본 주 작업자 슬롯 |
|---|---|---|
| `implementation` | task의 핵심 산출물 생산 | Implementer |
| `documentation` | 문서, 가이드, 참고자료 작성·갱신 | Communicator |
| `configuration` | 파라미터, 환경, 템플릿 설정·변경 | Operator |
| `design` | 구조적, 시각적, 방법론적 설계 생성·수정 | Implementer 또는 Design Authority |
| `review` | 기존 작업물에 대한 평가, 감사, 검토 수행 | Risk Assessor 또는 Verifier |
| `analysis` | 데이터, 시스템, 소스를 조사·탐색하여 인사이트 도출 | Implementer 또는 Verifier |
| `delivery` | 릴리스, 발행, 제출, 인계처럼 산출물을 다음 단계나 외부 주체에 넘기는 작업 | Operator |
...
최종 라우팅 책임은 Orchestrator에게 있다. Orchestrator는 `task_kind`를 출발점으로 삼되, `risk_level`, `scope.surfaces`, `gate_profile`, 현재 미션 맥락을 함께 보고 `routing.required_reviewer_slots[]`를 확정해야 한다.

라우팅은 두 단계로 정리한다. 먼저 `required_reviewer_slots[]`를 누적 계산하고, 그다음 구현체가 이를 `required_reviewer_types[]`로 매핑한다.
...
### 1단계 — `task_kind`별 기본 힌트

| task_kind | 최소 필수 리뷰어 슬롯 | 자주 추가되는 슬롯 |
|---|---|---|
| `implementation` | Design Authority | Verifier, Risk Assessor, Implementer (동료), Operator, Communicator |
| `documentation` | Communicator | Verifier, Design Authority (구조적 의미가 바뀐 경우) |
| `configuration` | Operator | Risk Assessor, Verifier, Design Authority |
| `design` | Design Authority | Communicator, Implementer, Verifier |
| `review` | Risk Assessor | Design Authority, Verifier, Operator |
| `analysis` | Design Authority, Verifier | Risk Assessor, Communicator |
| `delivery` | Operator, Verifier | Communicator, Risk Assessor |

이 단계의 결과는 `required_reviewer_slots[]`의 기본 집합이다. 이후 단계에서 Orchestrator가 이를 보수적으로 확장하거나 조정할 수 있다.
...
| `high` | Challenger, Risk Assessor | 독립 리뷰어 세트 + Challenger + 해당되는 경우 Risk Assessor |
| `critical` | Challenger, Risk Assessor, Verifier | 다관점 리뷰 (Challenger 포함). 단일 관점에만 의존한 종결은 허용하지 않는다 |
...
| 신호 | 추가할 리뷰어 슬롯 |
|---|---|
| 프레젠테이션 표면 — 사용자 대면 출력, 인터랙션 흐름, 시각 설계 | Implementer (프레젠테이션 도메인), Communicator |
| 핵심 로직 — 도메인 모델, 데이터 처리, 알고리즘, 인터페이스 | Implementer (로직 도메인), Design Authority |
| 신뢰 경계 — 인증 정보, 민감 데이터, 권한 | Risk Assessor |
| 운영 설정 — 산출물 인계 절차, 인프라, 환경, 런타임 설정 | Operator |
| 검증 표면 — 수용 기준, 커버리지 artifact, 재현성 | Verifier |
| 문서, 가이드, 사용자 대면 텍스트 | Communicator |

### 4단계 — 게이트 프로필 조정

- `closure_ready`에는 Verifier를 반드시 포함한다.
...
중복 리뷰어 슬롯을 제거한다. 그다음 구현체는 각 슬롯을 자기 환경의 구체 리뷰어 type에 매핑해 `required_reviewer_types[]`를 구성한다. `primary_worker_type`이 리뷰 세트에도 나타나면 해당 전문성이 필요하다는 뜻이지 자기 리뷰로 충분하다는 뜻이 아니다.

### 7단계 — 최소 보장

모든 task에는 `primary_worker_slot`과 다른 리뷰어 슬롯이 최소 하나 있어야 한다. type으로 매핑한 결과에서도 `primary_worker_type`과 다른 리뷰어 타입이 최소 하나 있어야 한다. 계산 결과가 worker 자신의 슬롯만 포함하거나 빈 집합이면 Design Authority를 추가한다.
...
| `eval_commands[]` | Verifier가 인수 기준을 검증할 때 실행하는 명령 |
| `rubric` | 게이트 평가에 사용되는 채점 항목과 가중치 |
| `retry_budget` | 남은 검증 재시도 횟수 |
| `scope` | `surfaces` 배열을 포함하는 객체 — 영향받는 표면, 경로, 도메인 |
| `routing` | `primary_worker_slot`, `primary_worker_type`, `required_reviewer_slots[]`, `required_reviewer_types[]`를 포함하는 객체 |
...
> **저장 위치**: 구현 계약은 별도 파일이 아니라 task root의 `record.json` `implementation_contract` 섹션에 저장한다.
...
> **저장 위치**: worker self-check는 task root의 `self-check.json`에 별도 artifact로 저장한다. 정규 형식은 `docs/schemas/self-check.schema.json`을 따른다.
...
| `implementing` | `reviewed` | `self-check.json` + `evidence/{agent}.json` implementer evidence |
| `reviewed` | `integrated` | `record.json` `gate_result` 섹션 (verdict=pass) + `evidence/{agent}.json` reviewer 또는 tester evidence |
