...
수용 기준은 충족 여부를 확인할 수 있을 만큼 구체적이어야 한다. 각 기준이 갖춰야 할 것:

- **확인 가능** — 통과했는지 안 했는지 누가 봐도 알 수 있다
- **범위가 명확** — 어디까지가 이 기준의 범위인지 분명하고, 끝없는 정리 작업으로 번지지 않는다
- **실패를 상상할 수 있음** — "이게 안 되면 이런 모습"을 구체적으로 떠올릴 수 있다
- **겹치지 않음** — 다른 기준을 다르게 말한 것이 아니다
- **서로 충돌하지 않음** — 다른 기준이나 알려진 제약과 모순되지 않는다
...
구현 계약은 worker와 리뷰어 사이에서 "무엇을, 어떻게 하고, 무엇을 건드리지 않을지"를 작업 전에 합의하는 문서다. worker, 리뷰어, 오케스트레이터 모두가 같은 기대치를 가진 상태에서 작업을 시작하기 위해 존재한다. 승인된 구현 계약 없이 task 구현을 시작할 수 없다.
...
- 승인된 `affected_surfaces` 밖의 영역을 변경
- 새로운 수용 기준이 추가되거나 기존 기준이 변경됨
- risk_level이 상승
- 새로운 외부 의존성이 발생
...
- 변경 대상이 실질적으로 확대됨
- 수용 기준이 실질적으로 바뀜
- 구조적 또는 사용자 흐름 가정이 바뀜
- 중요한 비목표가 범위 안으로 들어옴
- 선택한 검증 계획이 더 이상 적절하지 않음
- risk level이 바뀜
...
진행 속도를 유지하려고 차단에 해당하는 문제를 부채로 분류하는 것은 허용하지 않는다.

## 전이 테이블

| 이전 상태 | 다음 상태 | 필수 조건 |
|---|---|---|
| `drafted` | `ready` | task 컴파일 완료, baseline 유효, 메타데이터 완전 |
| `ready` | `implementing` | 구현 계약 승인, 작업 공간 준비 |
| `implementing` | `reviewed` | 구현 완료, worker self-check 존재, 필수 리뷰 존재 |
| `reviewed` | `integrated` | 통합 결과 생성 |
| `integrated` | `verified` | evidence gate 통과 |
| `verified` | `passed` | closure packet 완전, final verdict = `pass` |
...
| `verified` | `implementing` | 게이트 실패로 검증-수정 루프 |
...
Final verdict `iterate`는 제품 판단이며 게이트 실패와 다르다. `retry_budget`을 소진하지 않지만, 같은 task에 iterate가 반복되면 의사결정 자체에 문제가 있다는 신호다.
...
Geas에서 task는 단순한 할 일 목록이 아니다. 범위를 명확히 하고, evidence를 생산하며, 계획대로 되지 않을 때 보수적으로 복구하도록 강제하는 통제된 계약이다.
