...
- **쓰기**: Orchestrator가 회고 시 specialist review, challenge review, worker self-check의 memory_suggestions를 기반으로 작성
- **형식**: 자유 형식 마크다운, 갱신 시 전체 재작성
- **범위**: 프로젝트 전체, 미션과 세션을 넘어 지속

Agent memory는 Geas에서 가장 단순한 memory 메커니즘이다. 에이전트가 작업 전에 읽고, Orchestrator가 유용한 것을 학습한 후 갱신하는 파일이다.
