...
Task state는 이 task의 실행이 지금 어떤 agent로 진행 중이며 verify-fix를 몇 번 거쳤는지 가리키는 task 단위 runtime 인덱스다. Task contract의 `status` 필드(lifecycle state)와는 다르다. `status`는 "이 task가 어느 생애 단계에 있는지"를 가리키고, task state는 "이 task의 실행이 지금 어떤 agent로 묶여 있는지"를 가리킨다.
