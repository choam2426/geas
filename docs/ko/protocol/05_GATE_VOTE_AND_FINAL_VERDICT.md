# 05. Gate, Vote, Closure, Final Verdict

> **기준 문서.**
> Evidence gate, 루브릭 채점, vote round, closure packet 완전성, critical review challenge, task closure decision, mission final verdict를 정의한다.

## 목적

Geas는 자주 잘못 합쳐지는 네 가지 메커니즘을 분리한다:

- **Evidence Gate** — 객관적 검증
- **Vote Round** — 판단이 엇갈리거나 출시 리스크가 클 때의 구조화된 심의
- **Task Closure Decision** — task를 닫을지, 다시 열지, 에스컬레이션할지 정하는 운영 판단
- **Mission Final Verdict** — 미션 전체를 승인할지, 다시 열지, 에스컬레이션할지 정하는 최종 판단

이 분리는 필수다. 넷 중 하나가 다른 하나를 대체하는 것을 허용해서는 안 된다.

## 핵심 분리 원칙

1. 게이트는 evidence를 검증한다. 제품 전략을 판단하지 않는다.
2. Vote round는 이견이나 준비 상태를 심의한다. 빠진 evidence를 만들어내지 않는다.
3. Task closure decision은 완전한 패킷을 보고 판단한다. 직감만으로 결정하지 않는다.
4. `changes_requested`는 task closure decision 또는 mission final verdict 계층에 속한다. 게이트 계층이 아니다.
...
| 미션 설계 승인 | Decision Maker | full-depth 미션엔 Design Authority 리뷰 필수 |
...
| task closure decision | Orchestrator | 직관이 아닌 closure packet에 근거 |
| mission final verdict | Decision Maker | 미션 종결 근거에 기반한 최종 승인 |
...
|---|---|---|---:|
| `core_interaction` | 핵심 동작이 의도대로 작동하는가 | Verifier | 3 |
| `output_completeness` | 수용 기준을 빠짐없이 충족하는가 | Verifier | 4 |
| `output_quality` | 산출물의 구조적·방법론적 품질이 충분한가 | Design Authority | 4 |
| `regression_safety` | 기존 동작에 악영향을 주지 않는가 | Verifier | 4 |
...
시스템은 `self-check.json`을 단순 기록이 아닌 실질적 입력으로 활용해야 한다. Self-check가 직접 영향을 주는 영역:
...
| `readiness_round` | 종결 전 — 고위험 출시 준비 확인, 차단 challenge의 공식 처리, 완성도와 긴급성 간 트레이드오프 해소 | Orchestrator, Challenger, 관련 specialist (필요 시 Decision Maker) |
...
Decision Maker는 미션 수준의 수용 판단이나 상위 트레이드오프가 걸린 경우에 참여한다.
...
| `auto` | 촉발 조건이 충족되면 진행 |
| `always` | task closure decision 전에 항상 readiness round를 진행 |
...
- 미해소 리스크 존재
- 범위 밖 변경 발생
- 실패 또는 `changes_requested` 이후 재제출
...
Closure packet은 task의 전체 과정을 하나로 압축한 evidence 묶음이다. Orchestrator는 이 패킷만 보고 task closure decision을 내린다. 패킷이 불완전하면 종결 판단을 내릴 수 없다.
...
- 라우팅에서 요구된 필수 리뷰어 세트가 리뷰를 제출했거나
...
Task closure decision은 프로토콜에서 task를 종결할 수 있는 유일한 메커니즘이다. Evidence gate가 "검증을 통과했는가"를 판정한다면, task closure decision은 "이 task를 지금 닫을 수 있는가"를 판정한다. Orchestrator만이 이 판단을 내릴 수 있으며, closure packet에 근거해야 한다.

Orchestrator는 `decision` evidence를 남기고, task 수준 종결 상태는 해당 task의 `record.json`에 `verdict` 섹션으로 저장한다. 필드: `verdict` (`approved`/`changes_requested`/`escalated`), `rationale`, `rewind_target` (선택). 이 문서에서는 이 섹션을 task closure decision이라고 부른다.

### 판결 종류

| 판결 | 효과 | 재시도 한도 영향 |
|---|---|---|
| `approved` | task가 `passed` 상태가 된다 — 완료 | 없음 |
| `changes_requested` | 명시적 상태 복원 대상을 지정하고 추가 작업을 요구한다 | 소진하지 않음 |
| `escalated` | 현재 권한으로 결정할 수 없어 사용자에게 올린다 | 없음 |

### `approved`를 내릴 수 없는 조건

다음 중 하나라도 해당하면 `approved` 판결이 금지된다:
...
### `changes_requested` 규칙

`changes_requested`는 게이트 실패가 아니라 task closure decision이다. "검증은 통과했지만 이 task를 지금 닫기엔 부족하다"는 의미다. 재시도 한도를 소진하지 않지만 다음을 반드시 기록한다:
...
불확실성이 좁혀지지 않는 채 `changes_requested`가 반복되면 에스컬레이션을 촉발한다.

## Mission Final Verdict

Final verdict라는 말은 미션 단위에서만 쓴다. 이 판단은 개별 task가 아니라 미션 전체를 닫을 준비가 되었는지, 남은 debt와 gap, carry-forward 항목을 감안해도 최종 승인할 수 있는지를 본다. 이 판단의 소유자는 Decision Maker다.

Task closure decision이 모두 끝났다고 mission final verdict가 자동으로 따라오지는 않는다. 미션 final verdict는 gap assessment, debt register, phase review 같은 미션 종결 근거를 보고 별도로 내려야 한다.
...
성숙한 워크플로는 하나의 메커니즘에 모든 역할을 맡기지 않는다. Evidence gate는 검증하고, vote round는 심의하고, task closure decision은 task를 닫고, mission final verdict는 미션을 승인한다. 이 경계가 흐려지면 신뢰가 무너진다.
