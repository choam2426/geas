...
4. `changes_requested`는 final verdict 계층에 속한다. 게이트 계층이 아니다.
...
| 최종 판단 | Decision Maker | 직관이 아닌 closure packet에 근거 |
...
Decision Maker는 `decision` evidence를 남기고, task 수준 종결 상태는 해당 task의 `record.json`에 `verdict` 섹션으로 저장한다. 필드: `verdict` (`approved`/`changes_requested`/`escalated`), `rationale`, `rewind_target` (선택).
...
`changes_requested`는 게이트 실패가 아니라 최종 판단이다. "검증은 통과했지만 제품으로 받아들이기엔 부족하다"는 의미다. 재시도 한도를 소진하지 않지만 다음을 반드시 기록한다:
