...
| `worker_self_check` | worker의 구조화된 자기 점검 |
| `open_risks` | 인지했으나 완전히 해소하지 못한 리스크 |
| `debt_snapshot` | 이 task가 도입하거나 이월한 부채 |
| `readiness_round` | vote round 결과. 진행하지 않았으면 명시적 null |
| `challenge_review` | Challenger의 사전 출시 검증 결과. high/critical에서 필수, low/normal에서는 명시적 null |
