...
| [cleanup](#cleanup) | Utility | 불가 | Compass (MVP 이후 / Evolution) | `.geas/debt.json` 항목 |
...
Geas 오케스트레이터입니다. 멀티 에이전트 팀 전체를 관리합니다. 셋업, intake, 모드 감지, initiative/sprint/debate 프로토콜 위임까지 담당합니다.
...
- 시작 시 세 가지 모드 중 하나로 라우팅합니다: Initiative (새 제품), Sprint (범위 한정 기능), Debate (의사결정 전용).
...
**호출 주체:** Compass (시작 시퀀스 1단계, 모드 감지 전)
...
- Initiative 모드에서는 스코프 확정 전에 2~3개 접근 방식을 트레이드오프와 함께 제시합니다. Sprint 모드에서는 기능 범위 질문만 합니다.
...
3단계 품질 게이트입니다. EvidenceBundle을 TaskContract 기준으로 평가합니다. Mechanical(빌드/린트/테스트), Semantic(acceptance criteria + rubric), Product(Nova 판단).
...
**출력:**
- Gate 판정 (pass/fail/iterate) + tier별 결과
...
- Tier 2 (Semantic): worker evidence에서 acceptance criteria를 확인한 뒤, Forge와 Sentinel의 `rubric_scores`로 rubric 차원 점수를 비교합니다. 모든 차원이 임계값을 넘어야 합니다. worker의 `self_check.confidence`가 2 이하면 모든 임계값이 1 올라갑니다.
- Tier 3 (Product): Nova를 생성해 Ship/Iterate/Cut 판정을 받습니다. 기능 완성, 단계 완성, pivot 결정에만 실행됩니다. 중간 단계에서는 실행하지 않습니다.
...
Geas 팀으로 새 제품을 만듭니다. Discovery, MVP Build, Polish, Evolution 4개 단계입니다.
...
**핵심 동작:**
- MVP 작업마다 전체 11단계 파이프라인을 돌립니다. Design → Tech Guide → Implementation Contract → Implementation → Code Review → Testing → Evidence Gate → Critic Review → Nova Review → Retrospective → Resolve. Code Review와 Testing은 예외 없이 필수입니다.
- Ship Gate는 4개 필수 evidence 파일이 있어야 작업을 통과 처리합니다: `forge-review.json`, `sentinel.json`, `critic-review.json`, `nova-verdict.json`. 하나라도 빠지면 해당 단계를 실행합니다.
...
- Initiative MVP Build와 같은 11단계 파이프라인을 돌립니다. Ship Gate, Critic Review, Nova Review, Scrum 회고 모두 필수입니다.
...
구현 전에 기술 또는 제품 결정을 내리기 위한 구조화된 멀티 에이전트 토론입니다.
...
코드베이스 탐색 프로토콜입니다. 프로젝트 구조를 스캔하고, 스택을 감지하고, 아키텍처를 파악합니다. Sprint 모드에서 기존 상태가 없을 때 자동 실행됩니다.

**사용자 호출:** 불가

**호출 주체:** Compass — Sprint 모드에서 `.geas/memory/_project/conventions.md`가 없을 때
...
엔트로피 스캔입니다. AI가 만든 불필요한 코드, 사용하지 않는 코드, 컨벤션 드리프트를 찾습니다. 발견 사항을 `.geas/debt.json`에 기록합니다. MVP 이후 또는 Evolution 중에 실행합니다.

**사용자 호출:** 불가

**호출 주체:** Compass — Phase 2 (MVP) 이후, Phase 4 (Evolution) 중, 또는 사용자/Forge 명시 요청 시
