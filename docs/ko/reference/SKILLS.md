...
원본 출처는 `architecture/DESIGN.md §7.5`의 공통 카탈로그, `architecture/CLI.md`의 CLI 계약, 그리고 `plugins/geas/skills/{name}/SKILL.md`의 실제 skill 본문이다. 이 문서는 소비자 관점의 색인이며 CLI 계약이나 프로토콜 의미를 중복 기술하지 않는다.
...
그 외 `plugins/geas/skills/` 아래 skill은 모두 `mission`이 dispatch한다 — sub-skill의 frontmatter에 `user-invocable: false`가 있으므로 직접 호출하지 말 것.
...
- `plugins/geas/skills/{name}/SKILL.md` — skill별 실제 프롬프트 본문.
