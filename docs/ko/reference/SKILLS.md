...
| [orchestrating](#orchestrating) | Entry | 불가 | 사용자 (`/geas:mission` 경유) | 전체 세션 조율 |
...
| [vote-round](#vote-round) | Core - Verification | 불가 | Orchestrator (Discovery 시점) | `.geas/decisions/{dec-id}.json` |
| [initiative](#initiative) | Team - Execution | 가능 | Orchestrator 또는 사용자 | 전체 제품 빌드 (4단계) |
| [sprint](#sprint) | Team - Execution | 가능 | Orchestrator 또는 사용자 | 기존 프로젝트에 기능 추가 |
...
| [onboard](#onboard) | Team - Planning | 불가 | Orchestrator (Sprint, 최초 실행) | `.geas/memory/_project/conventions.md` |
...
| [cleanup](#cleanup) | Utility | 불가 | Orchestrator (Build phase 이후 / Evolution) | `.geas/debt.json` 항목 |
...
**호출 주체:** Orchestrator — Initiative (Discovery 1.6단계)와 Sprint (1단계)
...
**호출 주체:** Orchestrator — 주요 아키텍처 또는 횡단 관심사 결정 시 (주로 Discovery 1.5단계)
...
- `.geas/memory/_project/conventions.md`가 없으면(처음 Sprint하는 미지 프로젝트) Orchestrator가 Forge를 생성해 `onboard` skill을 먼저 실행합니다.
...
**호출 주체:** Orchestrator — delivery mode (Sprint 패턴)에서 `.geas/memory/_project/conventions.md`가 없을 때
...
- `conventions.md`가 이미 있으면 onboarding을 건너뜁니다. Orchestrator가 바로 읽고 Sprint 실행으로 넘어갑니다.
...
**호출 주체:** Nova — 마일스톤, Evolution 시작, Orchestrator/사용자 요청 시
...
**호출 주체:** Orchestrator — Initiative (Evolution 단계)와 Sprint 종료 시, 또는 사용자 요청 시
...
**호출 주체:** Orchestrator — Phase 2 (Build) 이후, Phase 4 (Evolution) 중, 또는 사용자/Forge 명시 요청 시
