...
| [decision](#decision) | Team - Execution | 가능 | Compass 또는 사용자 | `.geas/decisions/{dec-id}.json` |
...
- 실행 mission/mode를 직접 호출할 수도 있습니다 (`/geas:initiative`, `/geas:sprint`, `/geas:decision`).
...
Geas 오케스트레이터입니다. 멀티 에이전트 팀 전체를 관리합니다. 셋업, intake, mode 분기(discovery/delivery/decision), Initiative mission/Sprint 패턴/decision 프로토콜 위임까지 담당합니다.
...
decision mode로 구현 전에 기술 또는 제품 결정을 내리기 위한 구조화된 멀티 에이전트 decision입니다.
