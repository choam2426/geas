...
| [cleanup](#cleanup) | Utility | 불가 | Orchestrator (Build phase 이후 / Evolution) | `.geas/state/debt-register.json` 항목 |
...
- Build (작업별): 전체 evidence 체인 — `palette.json`, `forge.json`, `contracts/{id}.json`, `{worker}.json`, `forge-review.json`, `sentinel.json`, `critic-review.json`, `nova-verdict.json`, `tasks/{id}/retrospective.json`
...
- Scrum 회고는 모든 작업 후 필수입니다. 사소한 작업이라도 예외 없습니다. `.geas/tasks/{task-id}/retrospective.json`을 만듭니다. 파일이 없으면 1회 재시도합니다.
...
엔트로피 스캔입니다. AI가 만든 불필요한 코드, 사용하지 않는 코드, 컨벤션 드리프트를 찾습니다. 발견 사항을 `.geas/state/debt-register.json`에 기록합니다. Build phase 이후 또는 Evolution 중에 실행합니다.
