...
Geas는 **13개의 skill** (core 12개 + utility 1개)을 가진다. 도메인 비종속, 도구 비종속 skill로서 통제된 실행 파이프라인을 구현한다.
...
| Skill | 설명 | 핵심 역할 |
|-------|------|----------|
| `help` | 사용법 안내 및 명령 레퍼런스 | Geas 개념, 명령, 워크플로우를 설명한다 |

### help

Geas 사용법, 가용 명령, 워크플로우, 멀티 에이전트 프로세스를 설명한다. 대화형 출력만 생성하며 파일을 기록하지 않는다. `.geas/state/run.json`이 있으면 컨텍스트를 인식하여 응답한다.
