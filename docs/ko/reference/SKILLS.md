...
| [intake](#intake) | Core - Contract Engine | 불가 | Orchestrator | `.geas/spec/mission-{n}.json` |
...
| [write-prd](#write-prd) | Team - Planning | 불가 | Product Authority (Specifying 중) | `.geas/spec/prd.md` |
| [write-stories](#write-stories) | Team - Planning | 불가 | Product Authority (Specifying 중) | `.geas/spec/stories.md` |
...
| [cleanup](#cleanup) | Utility | 불가 | Orchestrator (Building phase 이후 / Evolving) | `.geas/evolution/debt-register.json` 항목 |
...
- `.geas/spec/mission-{n}.json` — intake 이후 미션 컨텍스트
- `.geas/ledger/events.jsonl` — 상태 전환 기록용 이벤트 로그
- `.geas/evolution/debt-register.json` — 에이전트 복귀 시마다 확인하는 기술 부채 추적

**출력:**
- `.geas/state/run.json` — 에이전트 생성 전마다 checkpoint 업데이트
- `.geas/ledger/events.jsonl` — 모든 상태 전환 이벤트 기록
- `.geas/evolution/debt-register.json` — evidence bundle에서 추출한 새 기술 부채 항목
...
- `.geas/evolution/debt-register.json` — 빈 기술 부채 레지스터 (`{"items": []}`)
...
**입력:**
- 사용자 자연어 (원본 미션 설명)
- `.geas/spec/mission-{n}.json` — 존재 여부 확인 (기존 프로젝트에서는 파일이 이미 있으면 건너뜁니다)

**출력:**
- `.geas/spec/mission-{n}.json` — `schemas/mission-spec.schema.json`을 따르는 확정된 미션 스펙
...
**입력:**
- 사용자 스토리 또는 기능 설명
- `.geas/spec/mission-{n}.json` — 미션 수준 컨텍스트
...
- `.geas/spec/mission-{n}.json` — 미션 컨텍스트
- `.geas/contracts/{task-id}.json` — implementation contract (Architecture Authority, QA Engineer 패킷용)
- `.geas/evolution/debt-register.json` — 해당 작업 관련 미해결 기술 부채
...
**입력:**
- Architecture Authority의 제안 (`.geas/decisions/pending/{proposal-id}.md`에 저장)
...
- `.geas/spec/mission-{n}.json` — 미션 컨텍스트와 확정된 스코프
...
- `.geas/evolution/debt-register.json` — 기술 부채 상태 (선택)
...
- `.geas/state/run.json`, `.geas/spec/mission-{n}.json` — `status` 쿼리 타입용
...
엔트로피 스캔입니다. AI가 만든 불필요한 코드, 사용하지 않는 코드, 컨벤션 드리프트를 찾습니다. 발견 사항을 `.geas/evolution/debt-register.json`에 기록합니다. Building phase 이후 또는 Evolving 중에 실행합니다.
...
**출력:**
- `.geas/evolution/debt-register.json` — 발견 사항별 새 항목 추가
...
**스키마:** 없음 (`.geas/evolution/debt-register.json`에 항목을 씁니다; 구조는 skill 내에서 정의됩니다).
...
**출력:**
- `.geas/decisions/{dec-id}.json` — Product Authority의 pivot 방향과 근거가 담긴 DecisionRecord
...
- `.geas/tasks/` — 충돌 감지용 `scope.paths`가 포함된 TaskContract
...
**입력:**
- `.geas/tasks/{task_id}/retrospective.json` — 회고에서 추출한 교훈
...
**입력:**
- `.geas/evolution/debt-register.json` — 부채 항목
- `.geas/evolution/gap-assessment-*.json` — 갭 평가
