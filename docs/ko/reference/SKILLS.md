...
| [task-compiler](#task-compiler) | Core - Contract Engine | 불가 | Compass | `.geas/tasks/{id}.json` |
| [context-packet](#context-packet) | Core - Contract Engine | 불가 | Compass | `.geas/packets/{task-id}/{worker}.md` |
| [implementation-contract](#implementation-contract) | Core - Contract Engine | 불가 | Compass | `.geas/contracts/{task-id}.json` |
| [evidence-gate](#evidence-gate) | Core - Contract Engine | 불가 | Compass | Gate 판정 + ledger 이벤트 |
| [verify-fix-loop](#verify-fix-loop) | Core - Contract Engine | 불가 | Compass (evidence-gate 경유) | 수정 반복 + DecisionRecord |
| [verify](#verify) | Core - Verification | 불가 | Worker 에이전트 | 콘솔 체크리스트 리포트 |
| [vote-round](#vote-round) | Core - Verification | 불가 | Compass (Discovery 시점) | `.geas/decisions/{dec-id}.json` |
| [initiative](#initiative) | Team - Execution | 가능 | Compass 또는 사용자 | 전체 제품 빌드 (4단계) |
| [sprint](#sprint) | Team - Execution | 가능 | Compass 또는 사용자 | 기존 프로젝트에 기능 추가 |
| [debate](#debate) | Team - Execution | 가능 | Compass 또는 사용자 | `.geas/decisions/{dec-id}.json` |
| [write-prd](#write-prd) | Team - Planning | 불가 | Nova (Discovery 중) | `.geas/spec/prd.md` |
| [write-stories](#write-stories) | Team - Planning | 불가 | Nova (Discovery 중) | `.geas/spec/stories.md` |
| [onboard](#onboard) | Team - Planning | 불가 | Compass (Sprint, 최초 실행) | `.geas/memory/_project/conventions.md` |
| [coding-conventions](#coding-conventions) | Utility | 불가 | 모든 에이전트 | 참조 가이드 전용 |
| [briefing](#briefing) | Utility | 불가 | Nova 또는 Compass | 콘솔 상태 리포트 |
| [run-summary](#run-summary) | Utility | 불가 | Compass (세션 종료 시) | `.geas/summaries/run-summary-<date>.md` |
| [ledger-query](#ledger-query) | Utility | 불가 | Compass 또는 사용자 | 포맷된 쿼리 결과 (읽기 전용) |
| [cleanup](#cleanup) | Utility | 불가 | Compass (Build phase 이후 / Evolution) | `.geas/debt.json` 항목 |
| [pivot-protocol](#pivot-protocol) | Utility | 불가 | Compass 또는 아무 에이전트 | `.geas/decisions/{dec-id}.json` |
...
- 실행 mission/mode를 직접 호출할 수도 있습니다 (`/geas:initiative`, `/geas:sprint`, `/geas:debate`).
...
Geas 오케스트레이터입니다. 멀티 에이전트 팀 전체를 관리합니다. 셋업, intake, mode 분기(discovery/delivery/decision), Initiative mission/Sprint 패턴/debate 프로토콜 위임까지 담당합니다.

**사용자 호출:** 불가 (`/geas:mission` 호출 시 내부적으로 `/geas:compass`가 실행됩니다)
...
- `.geas/debt.json` — 에이전트 복귀 시마다 확인하는 기술 부채 추적

**출력:**
- `.geas/state/run.json` — 에이전트 생성 전마다 checkpoint 업데이트
- `.geas/ledger/events.jsonl` — 모든 상태 전환 이벤트 기록
- `.geas/debt.json` — evidence bundle에서 추출한 새 기술 부채 항목
...
- 시작 시 세 가지 mode 중 하나로 라우팅합니다: discovery (새 제품 — Initiative 4-phase mission), delivery (범위 한정 기능 — Sprint 패턴), decision (의사결정 전용). 상세는 `docs/ko/protocol/02_MODES_MISSIONS_AND_RUNTIME.md` 참조.
...
**호출 주체:** Compass가 최초 실행 시(`.geas/state/run.json`이 없을 때) 호출합니다
...
- 사용자가 수동으로 실행할 필요 없습니다. Compass가 자동으로 트리거합니다.
...
**호출 주체:** Compass (시작 시퀀스 1단계, mode 분기 전)

**입력:**
- 사용자 자연어 (원본 미션 설명)
- `.geas/spec/seed.json` — 존재 여부 확인 (Sprint에서는 파일이 이미 있으면 건너뜁니다)
...
- Initiative mission에서는 스코프 확정 전에 2~3개 접근 방식을 트레이드오프와 함께 제시합니다. delivery mode (Sprint 패턴)에서는 기능 범위 질문만 합니다.
...
**호출 주체:** Compass — Initiative (Discovery 1.6단계)와 Sprint (1단계)
...
**호출 주체:** Compass가 worker를 작업에 배치하기 직전에 호출합니다
...
- `.geas/debt.json` — 해당 작업 관련 미해결 기술 부채
...
**호출 주체:** Compass — Tech Guide(Forge) 이후, Implementation 전. 모든 작업에 예외 없이 실행합니다
...
**호출 주체:** Compass — implementation, code review, QA 단계 이후마다 호출합니다
...
구조화된 리뷰 프로토콜입니다. Forge가 제안하고, Critic이 도전하고, Compass가 종합하며, 사용자가 확인합니다. DecisionRecord를 만듭니다.

**사용자 호출:** 불가

**호출 주체:** Compass — 주요 아키텍처 또는 횡단 관심사 결정 시 (주로 Discovery 1.5단계)
...
**핵심 동작:**
- 4단계로 진행합니다. Forge가 구조화된 제안(What / Why / Trade-offs / Alternatives) 작성 → Critic이 구조화된 도전(Assessment / Concerns / Alternative / Recommendation) 작성 → Compass가 양쪽을 종합해 사용자에게 옵션 제시 → 사용자 확인. 자율 모드에서는 Compass가 자동 결정합니다.
...
Geas 팀으로 새 제품을 만드는 4-phase mission입니다. Discovery, Build, Polish, Evolution 4개 phase입니다.

**사용자 호출:** 가능 (`/geas:initiative`)

**호출 주체:** Compass (mode 분기 후) 또는 사용자 직접

**입력:**
- `.geas/spec/seed.json` — intake에서 확정한 미션 스펙
- `.geas/tasks/` — Build phase 순서를 위한 컴파일된 TaskContract
...
- Build (작업별): 전체 evidence 체인 — `palette.json`, `forge.json`, `contracts/{id}.json`, `{worker}.json`, `forge-review.json`, `sentinel.json`, `critic-review.json`, `nova-verdict.json`, `memory/retro/{id}.json`
...
**핵심 동작:**
- Build phase 작업마다 전체 11단계 파이프라인을 돌립니다. Design → Tech Guide → Implementation Contract → Implementation → Code Review → Testing → Evidence Gate → Critical Reviewer Challenge → Final Verdict → Retrospective → Resolve. Code Review와 Testing은 예외 없이 필수입니다. 프로토콜 task state는 `drafted → ready → implementing → reviewed → integrated → verified → passed`입니다.
- Closure Packet + Critical Reviewer + Final Verdict 과정에서 4개 필수 evidence 파일이 있어야 작업을 통과 처리합니다: `forge-review.json`, `sentinel.json`, `critic-review.json`, `nova-verdict.json`. 하나라도 빠지면 해당 단계를 실행합니다.
- Scrum 회고는 모든 작업 후 필수입니다. 사소한 작업이라도 예외 없습니다. `.geas/memory/retro/{task-id}.json`을 만듭니다. 파일이 없으면 1회 재시도합니다.
...
delivery mode (Sprint 패턴)로 기존 프로젝트에 범위가 정해진 기능을 추가합니다.

**사용자 호출:** 가능 (`/geas:sprint`)

**호출 주체:** Compass (mode 분기 후) 또는 사용자 직접

**입력:**
- `.geas/spec/seed.json` — 읽기 전용 프로젝트 컨텍스트 (Sprint에서는 기존 파일을 수정하지 않습니다)
- `.geas/memory/_project/conventions.md` — 없으면 Forge가 먼저 onboarding합니다

**출력:**
- `.geas/tasks/{id}.json` — 기능 하나의 TaskContract
- Initiative Build phase와 동일한 전체 evidence 체인 (작업별)
- `.geas/summaries/run-summary-<date>.md` — 세션 감사 기록

**핵심 동작:**
- Discovery를 완전히 건너뜁니다. PRD, stories, 아키텍처 투표 없습니다. 기능 설명에서 바로 TaskContract 하나를 컴파일합니다.
- Initiative Build phase와 같은 11단계 파이프라인을 돌립니다. Closure Packet, Critical Reviewer Challenge, Final Verdict, Scrum 회고 모두 필수입니다.
- `.geas/memory/_project/conventions.md`가 없으면(처음 Sprint하는 미지 프로젝트) Compass가 Forge를 생성해 `onboard` skill을 먼저 실행합니다.
...
decision mode로 구현 전에 기술 또는 제품 결정을 내리기 위한 구조화된 멀티 에이전트 토론입니다.

**사용자 호출:** 가능 (`/geas:debate`)
...
- Compass가 입장을 종합하고, 트레이드오프를 제시하고, 사용자에게 최종 결정을 요청한 뒤, DecisionRecord를 씁니다.
...
**호출 주체:** Nova — Discovery (Initiative 1.3단계)
...
**호출 주체:** Nova — Discovery (Initiative 1.3단계), write-prd 직후
...
코드베이스 탐색 프로토콜입니다. 프로젝트 구조를 스캔하고, 스택을 감지하고, 아키텍처를 파악합니다. delivery mode (Sprint 패턴)에서 기존 상태가 없을 때 자동 실행됩니다.

**사용자 호출:** 불가

**호출 주체:** Compass — delivery mode (Sprint 패턴)에서 `.geas/memory/_project/conventions.md`가 없을 때
...
- `conventions.md`가 이미 있으면 onboarding을 건너뜁니다. Compass가 바로 읽고 Sprint 실행으로 넘어갑니다.
...
- 기술 스택 자체는 여기서 정하지 않습니다. Forge가 제안하고, Nova가 검증하고, Compass가 확인하며, DecisionRecord에 기록됩니다. 그 후 Forge가 프로젝트별 `conventions.md`를 씁니다.
...
**호출 주체:** Nova — 마일스톤, Evolution 시작, Compass/사용자 요청 시
...
**호출 주체:** Compass — Initiative (Evolution 단계)와 Sprint 종료 시, 또는 사용자 요청 시
...
- `costs.jsonl`이 있으면 Cost Report 테이블 (에이전트, 모델, phase, task별 생성 횟수)을 포함합니다. `debt.json`이 있으면 Tech Debt Report (심각도별 미해결, 이번 세션 신규/해결)를 포함합니다.
...
**호출 주체:** Compass 또는 사용자 — 진단, 상태 확인, 이력 조회용
...
엔트로피 스캔입니다. AI가 만든 불필요한 코드, 사용하지 않는 코드, 컨벤션 드리프트를 찾습니다. 발견 사항을 `.geas/debt.json`에 기록합니다. Build phase 이후 또는 Evolution 중에 실행합니다.

**사용자 호출:** 불가

**호출 주체:** Compass — Phase 2 (Build) 이후, Phase 4 (Evolution) 중, 또는 사용자/Forge 명시 요청 시
...
- 프로젝트 크기에 따라 스캔 깊이가 달라집니다. 소규모는 전체, 중규모는 현재 Sprint 변경 파일 + 코어 모듈, 대규모는 팀이 수정한 파일과 플래그된 영역만.
- 같은 근본 원인의 관련 발견 사항은 하나의 `debt.json` 항목으로 묶습니다. 한 파일에 주석 20개가 있다고 항목 20개를 만들지 않습니다.

**스키마:** 없음 (`.geas/debt.json`에 항목을 씁니다; 구조는 skill 내에서 정의됩니다).
...
**호출 주체:** Compass (evidence-gate의 `"pivot"` 에스컬레이션, Nova "Cut" 판정, 또는 팀원이 pivot 신호를 올릴 때)

**입력:**
- Compass의 전체 컨텍스트: 뭐가 잘못됐는지, 뭘 시도했는지, 가능한 옵션
