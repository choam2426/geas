...
**Orchestrator**는 미션 skill(메인 세션 드라이버)이지 스폰 가능한 agent가 아니다. Phase를 조율하고, specialist를 디스패치하고, task 종결 결정을 기록한다. `agents/`에는 등장하지 않는다.
...
소프트웨어 엔지니어링 미션용. `agents/software/`에 정의되어 있다.
...
연구·분석 미션용. `agents/research/`에 정의되어 있다.
