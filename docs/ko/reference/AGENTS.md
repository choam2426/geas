...
Discovery 시작 전에 Nova (1.2~1.3), Forge (1.4), Critic (1.5 vote round)이 전체 프로젝트의 기술 스택과 컨벤션을 정하는 아키텍처 투표에 참여합니다.
...
**주요 책임:**
- Discovery: write-prd와 write-stories skill로 미션 분석, 가치 제안, MVP 범위 정의.
...
**파이프라인 위치:**
- Discovery (1.2~1.3단계): intake 후 첫 번째로 생성되는 에이전트.
- 작업별 (2.9단계): Evidence Gate와 Critic 검토 후, Ship Gate 전.

**Evidence 출력:**
- `nova.json` (discovery) — 제품 비전, MVP 범위.
...
**주요 책임:**
- Discovery: 기술 스택 선택, `.geas/memory/_project/conventions.md` 작성.
...
**파이프라인 위치:**
- Discovery (1.4단계): 아키텍처 및 스택 결정.
- 작업별 (2.2단계): tech guide (조건 충족 시).
- 작업별 (2.3단계): implementation contract 승인.
- 작업별 (2.5단계): 필수 코드 리뷰.

**Evidence 출력:**
- `forge.json` (discovery/tech-guide) — 아키텍처 결정, 컨벤션, 기술적 접근.
...
**주요 책임:**
- Discovery: 대상 플랫폼(Vercel, Netlify, Docker)의 초기 빌드 및 배포 설정.
...
**주요 책임:**
- Discovery: 아키텍처 비판 (위험, 규모, 과잉 엔지니어링)과 제품 비판 (사용자 필요, 기존 대안, 최소 기능 세트).
...
**파이프라인 위치:**
- Discovery (1.5단계): 아키텍처 vote round. 항상 참여합니다.
- 작업별 (2.8단계): Evidence Gate 후, Nova 판정 전 필수 pre-ship 검토.

**Evidence 출력:**
- `vote-critic.json` (discovery vote) — 위험, 근거, 대안, 트레이드오프가 담긴 투표.
...
| 모드 | 목적 | 참여 에이전트 |
|------|------|-------------|
| **Initiative** | 처음부터 새 제품을 4단계로 빌드 | 12명 전원: Discovery (Nova, Forge, Critic), MVP Build (작업별 전체 파이프라인), Polish (Shield, Scroll), Evolution |
