...
6개 lifecycle event에 걸쳐 13개 hook (12개 스크립트 + 1개 공유 라이브러리).
...
| 8 | PostToolUse | Write\|Edit | checkpoint-post-write.sh | 쓰기 후 pending checkpoint 정리 |
| 9 | PostToolUse | Write\|Edit | packet-stale-check.sh | 오래된 context packet 경고 |
| 10 | PostToolUse | Bash | integration-lane-check.sh | integration lock 없이 merge 시 경고 |
| 11 | Stop | (all) | calculate-cost.sh | 세션 비용 계산 |
| 12 | PostCompact | (all) | restore-context.sh | 컨텍스트 압축 후 상태 복원 |
...
7. `active` 상태 항목 중 `review_after` 날짜가 지난 것을 확인한다.
...
`checkpoint-post-write.sh` (Hook 8)와 함께 동작한다.
...
| `.geas/ledger/events.jsonl` | (CLI를 통한 이벤트 로깅) |
