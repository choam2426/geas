...
쓰여진 파일이 `*/.geas/spec/mission-*.json`이면 stderr에 경고합니다:
```
...
1. **`.geas/evolution/debt-register.json`에만 반응합니다.** 나머지는 무시합니다.
...
- `.geas/evolution/debt-register.json`에 쓸 때만 동작합니다.
...
| `.geas/spec/mission-{n}.json` | protect-geas-state (동결 보호) |
