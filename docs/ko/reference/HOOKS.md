...
`.geas/missions/<mission_id>/tasks/<current_task_id>.json`에서 현재 작업의 `scope.paths` 허용 목록을 읽습니다. 쓰여진 파일이 해당 glob 패턴 중 어느 것과도 일치하지 않으면(`fnmatch` 사용) stderr에 경고합니다:
```
...
쓰여진 파일이 `*/.geas/missions/*/spec.json`이면 stderr에 경고합니다:
```
...
- 미션 스펙 경고는 내용과 무관하게 해당 경로 패턴에 쓸 때마다 발생합니다.
...
1. **`.geas/missions/*/tasks/*.json` 파일에만 반응합니다.** 나머지는 무시합니다.
...
- `.geas/missions/*/tasks/*.json`에 쓸 때만 동작합니다.
...
1. **`.geas/missions/<mid>/evolution/debt-register.json`에만 반응합니다.** 나머지는 무시합니다.
...
- `.geas/missions/<mid>/evolution/debt-register.json`에 쓸 때만 동작합니다.
...
1. **`.geas/missions/*/packets/*.md` 파일에만 반응합니다.** 나머지는 무시합니다.
...
- `.geas/missions/*/packets/*.md`에 쓸 때만 동작합니다.
- `.geas/state/memory-index.json`이 없으면 건너뜁니다.
- 비활성 상태의 메모리만 플래그합니다.
...
3. **현재 작업의 컨텍스트 패킷이** `.geas/missions/<mission_id>/packets/<task_id>/` 아래에 있는지 확인합니다.
...
| `.geas/missions/{mission_id}/spec.json` | protect-geas-state (동결 보호) |
