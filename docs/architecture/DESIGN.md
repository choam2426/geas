...
- `events.jsonl`: an audit log appended by the CLI for every mutation command that advances the protocol — state transitions, approvals, artifact appends, and single-artifact set operations (`mission design-set`, `impl-contract set`, `memory shared-set` / `agent-set`, `gap set`, `memory-update set`). Event logging is best-effort: the primary artifact write is atomic and authoritative, and an event-append failure is swallowed rather than rolling the primary write back. `events.jsonl` is not a protocol artifact, but the implementation must maintain it. Each line is an independent JSON object (`kind`, `actor`, `triggered_by`, `prior_event`, `created_at`, and so on). If an implementation records runtime-boundary events, entries such as `SessionStart` and `SessionEnd` may be appended here as well. The owner of the logged shapes and rules is `CLI.md` Section 14.6; the authoritative kind table lives in `HOOKS.md` §4.2.
...
| `consolidating-mission` | `orchestrator` (spawns `design-authority` for gap) | Entering consolidating phase | `consolidation scaffold`, `debt register`, `gap set`, `memory-update set`, `memory shared-set`, `memory agent-set` |
