...
subagent가 완료된 후, 이 hook은 agent spawn metadata 로깅을 수행합니다 -- agent 이름, 작업 ID, model을 `.geas/ledger/costs.jsonl`에 기록하여 분배 분석에 사용합니다.
...
**목적:** 세션 종료 시 subagent 세션 JSONL 파일을 파싱하여 실제 토큰 사용량과 예상 비용을 계산합니다.

**단계별 동작:**

1. `cwd`에서 Claude Code project directory를 유도합니다 (예: `A:\project` → `~/.claude/projects/A--project/`).
2. `subagents/` 폴더가 있는 가장 최근 세션 directory를 찾습니다.
3. 모든 subagent `.jsonl` 파일을 파싱합니다 -- 모든 assistant 메시지에서 `input_tokens`, `output_tokens`, `cache_creation_input_tokens`, `cache_read_input_tokens`를 합산합니다.
4. Opus 가격을 상한선으로 사용하여 예상 비용을 계산합니다.
5. 토큰 합계, 비용 내역, agent별 분배를 포함한 `.geas/ledger/cost-summary.json`을 작성합니다.
6. stderr에 세션 비용 요약을 출력합니다.

**출력:** `.geas/ledger/cost-summary.json`

**경고 메시지:**

| 메시지 | 의미 |
|--------|------|
| `Session cost: $X.XX (N agents, M output tokens)` | 종료 시 세션 비용 요약 |
...
[Stop] ---> verify-pipeline.sh
    ...
    |
    +------> calculate-cost.sh
    |          - subagent JSONL 파일에서 토큰 사용량 파싱
    |          - cost-summary.json 작성
    |          - 세션 비용 요약 출력
    |          - (차단하지 않음)
    |
    |--- exit 0 ---> 세션 종료
...
- **agent-telemetry**는 subagent 완료 후 agent, task, model 정보를 기록합니다. 이 데이터는 run-summary 비용 보고서에 사용됩니다.
...
- **calculate-cost**는 세션 종료 시 subagent 세션 JSONL 파일을 파싱하여 실제 토큰 사용량과 예상 비용을 계산하고, `.geas/ledger/cost-summary.json`에 요약을 작성합니다.
...
| `.geas/ledger/costs.jsonl` | agent telemetry 로그 (agent-telemetry가 기록, run-summary가 읽음) |
| `.geas/ledger/cost-summary.json` | 토큰 사용량 및 비용 요약 (calculate-cost가 작성) |
