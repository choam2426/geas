...
| [context-packet](#context-packet) | worker agent를 위한 역할 맞춤형 briefing 생성 | worker 디스패치 전 `compass`가 호출 | TaskContract, 이전 에비던스, Linear 스레드, seed 스펙 | `.geas/packets/{task-id}/{worker}.md` |
...
| skill | 설명 | 호출 방식 | 입력 | 출력 |
|-------|------|-----------|------|------|
| [linear-cli](#linear-cli) | Linear API를 위한 Python CLI 래퍼 | Linear 작업이 필요한 모든 skill 또는 agent가 호출 | 명령어 + 플래그 (예: `create-issue --title ...`) | Linear API 응답 (JSON) |
| [linear-protocol](#linear-protocol) | 스타트업 특화 Linear 사용 규칙 -- label, comment 형식, workflow 규칙 | agent와 skill이 서식 가이드로 참조 | N/A (참조 문서) | N/A (표준 정의) |
| [setup](#setup) | 최초 설정 -- Linear API 키 검증, 의존성 확인, Linear 워크스페이스 구성, 설정 파일 생성 | 최초 실행 시 `compass`가 `/geas:setup`을 통해 호출 | 사용자 입력 (Linear API 키, 팀 선택) | `.geas/` directory 구조, `.geas/config.json`, `.geas/rules.md` |

### 유틸리티

| skill | 설명 | 호출 방식 | 입력 | 출력 |
|-------|------|-----------|------|------|
| [briefing](#briefing) | 제품 건강 상태에 대한 Nova의 구조화된 상태 보고 | Nova agent skill; milestone, phase 전환 시 또는 요청 시 | 실행 상태, Linear 이슈, 이전 briefing | 상태 보고서 (Linear comment + 콘솔) |
| [cleanup](#cleanup) | AI 슬롭, 데드 코드, convention 이탈을 탐지하는 엔트로피 scan | Forge agent skill; MVP 이후 또는 Evolution 중 | source 파일, conventions.md | Linear에 기술 부채 이슈 |
...
| [pivot-protocol](#pivot-protocol) | 현재 접근 방식이 실패할 때의 전략적 방향 전환 | 반복적 실패, Nova "Cut" 판정, agent 우려 시 트리거 | 실패 context, 에비던스, 옵션 | Nova의 피벗 결정, 재구성된 Linear 보드 |
| [run-summary](#run-summary) | 세션 종료 요약 생성 -- 결정사항, 완료된 이슈, agent 통계, verify-fix loop. Linear Document와 콘솔에 게시 | initiative Phase 4 끝 / sprint 끝에 compass가 호출, 또는 요청 시 | 실행 상태, agent 로그, Linear 이슈 | Linear Document + 콘솔 출력 |
...
| **입력** | TaskContract, 업스트림 worker의 이전 에비던스, Linear 스레드(활성화된 경우), seed 스펙. |
...
통과 시: TaskContract 상태 갱신, event 기록, Linear comment 게시.
...
| phase | 주요 활동 |
|--------|----------|
| **Genesis** | Seed 확인, Linear bootstrap, Nova vision, PRD & 사용자 story (Nova), Forge architecture, vote round (Critic 필수 참여), story 기반 TaskContract 컴파일, MCP server 추천 |
...
| **이름** | linear-cli |
| **카테고리** | 서피스 (협업) |
| **설명** | Linear 워크스페이스 CLI -- 이슈, project, 문서, 팀, 사용자, comment, label, cycle, milestone, 첨부 파일을 커버하는 Linear API용 Python 래퍼. |
| **호출 시점** | Linear과 상호작용이 필요한 모든 skill 또는 agent가 호출합니다. 모든 Linear 작업은 이 CLI를 통해 수행됩니다. |
...
| **이름** | linear-protocol |
| **카테고리** | 서피스 (협업) |
| **설명** | 스타트업 특화 Linear 사용 규칙 -- label, comment 형식, workflow 규칙. |
| **호출 시점** | agent와 skill이 서식 및 workflow 가이드로 참조합니다. 절차적 skill이 아닙니다. |
| **입력** | N/A (참조 문서). |
| **출력** | N/A (표준 정의). |

주요 규칙:
- comment 형식: `[AgentName] content`
- workflow: Backlog -> Todo -> Waiting -> In Progress -> In Review -> Testing -> Done
- 이슈 제목 형식: `[Label] Short description`
- label: type (feature, bug, design-spec, ...), area (frontend, backend, infra), role (needs-review, needs-qa)
- 추정치: Fibonacci 스케일 (1, 2, 3, 5, 8, 13)
...
설정 단계:
1. **Phase A**: `.geas/` 하위 directory 생성 (spec, state, tasks, packets, evidence, decisions, ledger, memory). 초기 `run.json`과 `rules.md` 작성.
2. **Phase B** (선택): Linear 설정 -- API 키 감지 또는 요청, 팀 선택, label 및 workflow 상태 생성, 설정 저장.
...
| **입력** | `.geas/state/run.json`, Linear 이슈 및 comment, 이전 briefing. |
| **출력** | Linear comment로 게시되고 콘솔에 출력되는 briefing. |
...
| **이름** | cleanup |
| **카테고리** | 유틸리티 |
| **설명** | 엔트로피 scan -- AI 슬롭, 미사용 코드, 데드 코드, 중복, 과도한 추상화, convention 이탈을 탐지합니다. Linear에 기술 부채 이슈를 생성합니다. |
| **호출 시점** | Phase 2 (MVP) 이후, Phase 4 (Evolution) 중, 또는 명시적 요청 시. |
| **입력** | project source 파일, `.geas/memory/_project/conventions.md`. |
| **출력** | `tech-debt` label이 붙은 Linear 이슈, project 추적 이슈에 요약 comment. |
...
| **이름** | run-summary |
| **카테고리** | 유틸리티 |
| **설명** | 세션 종료 요약 생성 -- 결정사항, 완료된 이슈, agent 통계, verify-fix loop. Linear Document와 콘솔에 게시합니다. |
| **호출 시점** | initiative Phase 4 끝과 sprint 끝에 compass가 호출. 세션 핸드오프 전 또는 명시적 요청 시에도 호출. |
| **입력** | `.geas/state/run.json`, `.geas/memory/_project/agent-log.jsonl`, Linear 이슈 및 comment. |
...
| **출력** | 항목별 판정 (PASS/FAIL/SKIP/PENDING)과 전체 VERDICT. Linear comment로 게시. |
