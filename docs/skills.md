...
이 문서는 Geas Skill 체계의 기준 문서다. Skill의 종류, 책임 경계, 입력, 출력, artifact 기록 지점, handoff 기준, resource 작성 기준을 정한다.
...
Skill은 User가 결과를 검토하고 수용 판단할 수 있는 상태를 만들기 위한 절차다. Skill은 계약, 검증 근거, 미검증 범위, User briefing, 기록 지점을 분리해 User의 수용 판단 비용을 낮춘다.

Evidence는 agent가 준비하는 검증 근거다. User Judgment는 User가 Evidence와 미검증 범위, 남은 위험을 검토한 뒤 내리는 결정이다. Evidence verdict, recommendation, Task 요약은 User Judgment의 입력이 될 수 있지만 User Judgment가 아니다.

### 독립 실행과 안전 정지

각 Skill은 자기 Skill 폴더 안의 `SKILL.md`, `references/`, `scripts/`, `assets/`만으로 실행 흐름을 이해하거나 안전하게 멈출 수 있어야 한다.
...
Mission Plan, Pre-build Design Surface 결정, Task Contract는 `specifying`이 작성하고 조율하는 기준 흐름이며, 이 role handoff 규칙의 대상이 아니다.
...
| 종류 | 책임 |
| --- | --- |
| `entrypoint` | User 요청을 받아 Mission 시작, 재개, 이어받기 맥락 점검, 다음 절차 선택을 조율한다. |
| `stage` | Mission 안의 큰 작업 단계를 조율하고 User briefing, artifact 기록 시점, 다음 단계 전환을 준비한다. |
...
| Skill | 종류 | 존재 이유 |
| --- | --- | --- |
| `mission` | `entrypoint` | Mission 시작, 재개, 이어받기 맥락 점검, stage 전환, User briefing을 하나의 진입점에서 조율한다. |
| `specifying` | `stage` | User 목표를 Mission Spec, Mission Plan, Pre-build Design Surface, Task Contract로 검토 가능한 기준과 실행 전 판단 표면으로 만든다. |
| `building` | `stage` | Task 실행 loop를 조율하고 Task 수용 판단용 briefing과 Task Evidence 기록으로 이어지게 한다. |
...
| `consolidating` | `stage` | 수용된 Task Evidence를 Mission 기준으로 종합하고 Mission 수용 판단용 briefing, debt 후보, memory 후보를 준비한다. |
...
mission
  -> specifying
     -> Mission Spec
     -> Pre-build Design Surface (plan-level, 필요 시)
     -> Mission Plan
  ...
  -> building
     ...
     -> Task 수용 판단용 briefing
     -> User Judgment
     -> Task Evidence

  -> consolidating
     -> Mission 수용 판단용 briefing
     -> User Judgment
     -> Debt / Memory candidates
     -> Mission Evidence
```

흐름은 User Judgment에서 멈춘다. Skill은 판단용 briefing을 준비하고, User는 수용, 제한부 수용, 재작업, 취소를 결정한다.
...
- Mission Spec, Mission Plan, Task Contract, Evidence, User Judgment, Debt Ledger, Memory, Continuity Ledger를 복귀 기준으로 읽는다.
- 다음 stage Skill을 선택한다.
- 진행 가능한 stage가 없거나 기준이 불명확하면 stop condition을 드러낸다.
- User에게 현재 상태, 다음 행동, 필요한 결정을 briefing한다.
- Mission 종료가 필요하면 `consolidating`으로 연결한다.

간단 흐름:

1. User 요청과 이어받기 맥락을 읽는다.
2. 수용된 Mission 기준과 진행 중인 Task 기준을 확인한다.
...
- Mission Plan이나 Task Contract 작성이 필요하면 `specifying`으로 연결한다.
- User Judgment는 User 결정에서만 온다.

입력:

- User 요청
- 최신 Mission Spec, Mission Plan, Task Contract
- 관련 Evidence와 User Judgment
- Debt Ledger, Memory, Continuity Ledger
...
- Mission 시작 시 수용된 Mission 기준을 기록한다.
...
`specifying`은 User 목표를 Mission Spec, Mission Plan, 필요한 Pre-build Design Surface, 초기 Task Contract로 이어지는 검토 가능한 기준으로 만든다.
...
- Mission Plan draft를 만들고 User review로 올린다.
- Mission Spec 수용 뒤 Mission Plan 수용 전에 plan-level Pre-build Design Surface가 필요한지 판단한다.
- Mission Plan 합의 이후 Task Contract Set 수용 전에 contract-level Pre-build Design Surface가 필요한지 판단한다.
- Pre-build Design Surface 필요 여부를 판단할 때 result, data, workflow, export, acceptance, operational 관점을 각각 분류해 빠뜨리기 쉬운 기준 결정을 찾는다.
...
- User가 수용한 Mission Spec, Mission Plan, 초기 Task Contract Set을 기록한다.
...
4. User가 Mission Spec을 수용하면 Mission Plan 후보를 만들고 User review로 올린다.
5. User가 Mission Spec을 수용하면 Mission Plan을 잠그기 전에 plan-level Pre-build Design Surface 필요 여부를 판단하고, 필요한 경우 User가 Mission 접근, 산출물 형태, 외부 contract, domain/structural model, 큰 tradeoff를 판단하게 한다.
6. 선택된 plan-level 결정을 반영해 Mission Plan 후보를 만들고 User review로 올린다.
7. Mission Plan 합의 이후에는 Task Contract를 잠그기 전에 contract-level Pre-build Design Surface 필요 여부를 판단하고, 필요한 경우 Task slicing, 산출물, 수용 기준, verification checks, dependency, 첫 building Task를 판단하게 한다.
8. 선택된 contract-level 결정을 반영해 초기 Task Contract 후보를 만들고 User review로 올린다.
9. User가 수용한 기준만 runtime에 기록한다.

경계:

- Mission Spec 수용은 Mission Plan 수용을 뜻하지 않는다.
- Mission Plan 수용은 contract-level Pre-build Design Surface 결정이나 초기 Task Contract 수용을 뜻하지 않는다.
- Pre-build Design Surface는 runtime artifact, Evidence, User Judgment가 아니며, 선택된 결정이 Mission Plan이나 Task Contract Set에 반영될 때 기준이 된다.
- Task 분리와 dependency는 초기 Task Contract에서 정하고, Mission Plan은 Task graph의 정본이 아니다.
- implementation, verification, review, challenge Evidence는 담당하지 않는다.
- 하나의 User 응답으로 Mission Spec, Mission Plan, Pre-build Design Surface 결정, 초기 Task Contract Set을 한꺼번에 수용한 것으로 처리하지 않는다.

입력:

- User 목표와 대화 맥락
- 프로젝트 관찰 사실
- 새 Mission이면 이전 기준 없음
- 기준 갱신이면 accepted Mission Spec, Mission Plan, Task Contract
- 기준 갱신이면 관련 Evidence와 User Judgment
- User가 명시한 결정과 위임 범위
...
- User가 Mission Plan을 수용한 뒤 Mission Plan을 기록한다.
...
| `references/mission-plan.md` | `reference` | Mission Plan 작성 | plan payload shape와 review 기준이 길다. |
| `references/pre-build-design-surface.md` | `reference` | Mission Spec 수용 뒤, 결정을 잠글 기준 artifact 수용 전 | HTML, diagram, prototype, comparison 같은 임시 판단 표면의 trigger, 생략 조건, Mission Plan/Task Contract 반영 기준이 필요하다. |
| `references/task-contract.md` | `reference` | Task Contract 작성 | Task 분리, acceptance, verification, review focus 기준이 길다. |
| `references/baseline-readiness.md` | `reference` | building 진입 전 | 기준 완성도 점검과 User briefing shape가 반복된다. |
| `scripts/` | `none` | 없음 | intake와 기준 작성은 판단 중심이며 결정적 계산으로 분리할 부분이 확인된 뒤 만든다. |
...
- current Task Contract를 읽는다.
- Task Workflow에서 필요한 role-producing Skill을 호출한다.
- Implementation, Verification, Review, optional Challenge 결과를 모아 Task 수용 판단용 briefing을 준비한다.
...
1. current Task Contract와 관련 Evidence를 읽는다.
2. 필요한 role-producing Skill을 호출하고 handoff packet을 전달한다.
3. role Evidence를 모아 Task 수용 판단용 briefing을 준비한다.
...
| Resource | Type | 읽히는 단계 | 분리 이유 |
| --- | --- | --- | --- |
| `references/task-loop.md` | `reference` | Task Workflow 선택 | Task 단계별 dispatch와 recovery 기준이 반복된다. |
...
| `references/task-acceptance-input.md` | `reference` | User 판단용 briefing | Evidence 요약, 미검증 범위, 선택지 briefing shape가 길다. |
...
| `scripts/` | `none` | 없음 | Task 단계 선택은 User 결정과 Evidence에 묶여 있어 script보다 절차 기준이 우선이다. |
...
1. `read_first`에 있는 기준과 Task Contract를 읽는다.
...
`challenging`은 기준과 Evidence가 놓친 위험을 압박한다.
...
`consolidating`은 Mission 수용 판단용 briefing을 준비한다.

책임:

- 수용된 Task Evidence를 Mission Spec과 Mission Plan에 대조한다.
- Mission acceptance criteria별 결과와 근거를 정리한다.
- gap, 수용된 Task들의 미검증 범위, 남은 위험, follow-up 후보를 드러낸다.
- Debt Ledger 후보와 Memory 후보를 준비한다.
- Mission 수용 판단용 briefing을 User briefing으로 올린다.
...
| Resource | Type | 읽히는 단계 | 분리 이유 |
| --- | --- | --- | --- |
| `references/mission-acceptance-input.md` | `reference` | Mission 수용 판단용 briefing | criteria별 종합과 gap 표현 shape가 길다. |
...
Mission 안에서 실행되는 role은 존재하는 최신 accepted Mission Spec과 Mission Plan을 `read_first`에 포함한다.
...
| Evidence payload | agent가 준비한 검증 근거 | 해당 role 또는 stage 책임자가 작성하고 기록한다. |
| User Judgment payload | User 결정 원본 | User가 결정한 뒤 기록한다. |
| Runtime record | 수용된 기준, Evidence, User Judgment, ledger를 남긴 artifact | 본문 의미는 해당 책임자가 작성하고 기록은 판단을 대체하지 않는다. |
...
| `scripts/` | 반복적이고 결정적인 검증, 정렬, schema 확인 같은 실행 작업 |
...
- artifact 위치만 보고 완료를 선언하는 판단 reference
...
| 결정 | 선택지 | 기준 |
| --- | --- | --- |
| role prompt 배치 | stage-owned reference / role-producing Skill reference / 별도 role catalog | 각 Skill이 자기 폴더 리소스만으로 실행하거나 안전하게 멈출 수 있는 구조를 선택한다. |
