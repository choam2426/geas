...
CLI는 runtime에서 파생되는 identifier와 파일명을 할당한다. Mission id와 Evidence 번호는 CLI가 생성한다. Task id는 Mission Design의 `task_breakdown`에 기록된 값을 기준으로 한다.
...
- Mission Design의 `task_breakdown.task_id`가 중복되지 않는다.
- Task dependency가 존재하는 Task를 가리킨다.
- Task dependency가 순환하지 않는다.
...
- 진입할 Task id가 현재 Mission Design에 존재한다.
...
- Mission Design의 대상 Task들이 Task Evidence를 가진다.
- 대상 Task들이 수용된 Task result User Judgment를 가진다.
