...
Task 분리와 dependency는 Task contract의 책임이다. Mission design은 Task graph의 정본이 아니다.

User는 Mission spec이 자신의 목표와 맞는지, Mission design이 이해 가능한 진행 계획인지, 초기 Task contract가 실행 기준으로 충분한지 확인하고 받아들이거나 수정한다. 합의된 기준선은 runtime에서 versioned Mission Spec, Mission Design, Task Contract로 남긴다.
