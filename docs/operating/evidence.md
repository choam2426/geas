...
Mission 단위 Evidence 흐름은 다음과 같다.

```text
Task Evidence들
  + 필요한 role별 Evidence
  + Mission Spec / Mission Design
  -> Mission 수용 판단 입력
  -> User Judgment
  -> Memory 반영
  -> Mission Evidence
...
Task 수용 판단 입력은 runtime artifact가 아니라 User에게 제시하는 판단 묶음이다. 이 묶음에는 보통 Task Contract, 산출물, Implementation Evidence, Verification Evidence, Review Evidence, 필요한 Challenger Evidence, 미검증 범위와 남은 위험 요약, User가 선택할 수 있는 판단 선택지가 들어간다.

Mission 수용 판단 입력도 runtime artifact가 아니다. 이 묶음에는 보통 Mission Spec, Mission Design, 수용된 Task Evidence, 필요한 role별 Evidence, gap, debt, follow-up 후보, 받아들일 수 있는 미검증 범위와 남은 위험 후보, User가 선택할 수 있는 판단 선택지가 들어간다.

Role Evidence는 Task 수용 판단 전에 각 role이 자기 책임 범위에서 남기는 판단 입력이다. Task Evidence는 User가 Task 결과를 판단한 뒤 남기는 종료 요약이다. Mission Evidence는 User가 Mission 결과를 판단하고 회고와 Memory 반영을 정리한 뒤 남기는 종료 요약이다.

Evidence는 User Judgment가 아니다. Evidence verdict는 User 판단을 돕는 agent 측 입력이다. 미검증 범위는 검증 근거가 아니다. Task Evidence는 role별 Evidence를 대신하지 않고, Mission Evidence는 Task Evidence를 대신하지 않는다.
...
contract delta는 실행 중 Task contract와 달라졌거나 갱신이 필요한 지점을 드러내는 데 둔다. 조용히 실행 범위를 넓히는 대신, 갱신된 Task contract나 결과 수용 판단으로 이어질 수 있게 남긴다.
...
Challenger Evidence는 Challenger가 Task Contract, 산출물, Implementation Evidence, Verification Evidence, Review Evidence를 devil's advocate 관점과 장기 운영 관점에서 압박해 Task 수용 판단 전에 놓치기 쉬운 위험을 finding과 근거로 드러내는 Evidence다.

Mission 기준선 압박 결과는 User와 합의된 Mission Spec, Mission Design, Task Contract에 반영한다.

Challenger Evidence는 Task 수용 판단 전에 숨은 가정, 검증 공백, 암묵적 scope 확장, 장기 유지보수 위험을 드러낸다.
