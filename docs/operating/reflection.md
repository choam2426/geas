...
Memory는 적용 범위에 따라 두 수준으로 나눈다.

|수준|의미|
|---|---|
|공통 memory|Orchestrator를 포함한 모든 agent가 이후 작업에서 따라야 할 운영 기준|
|role별 memory|특정 role이 자기 책임 범위에서 이후 작업에 적용해야 할 운영 기준|
...
- 데이터 삭제, 마이그레이션, 권한 변경처럼 되돌리기 어려운 작업에서는 rollback 방법과 검증 방법을 계약에 포함한다.
