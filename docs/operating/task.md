...
Task는 User가 그 단위만 보고 완료 수용, 재작업, 보류, 중단을 판단할 수 있어야 한다. 이 문서에서는 이런 단위를 판단 가능한 작업 단위(reviewable unit)로 본다. 이를 위해 Task는 산출물이 분명하고, 수용 기준과 확인 방법이 있으며, 한 번에 검토 가능한 크기이고, 확인하지 못한 범위와 남은 위험을 Evidence에서 따로 드러낼 수 있어야 한다.
...
Task 분리는 Task contract의 책임이다. Task를 나눌 때는 agent가 실행하기 편한 순서보다 User가 판단하기 쉬운 경계를 우선한다.

## Task Contract

Task contract는 Task 분리, Task가 맡는 Mission의 일부, 수행 범위, 산출물, 수용 기준, verification checks, review focus, 위험 수준을 고정하는 실행 계약이다.
...
Task-level Pre-build Design Surface에서 User가 선택한 구현 전 결정은 Task contract에 반영될 때 실행 기준이 된다. HTML, diagram, prototype, comparison 같은 design surface 자체는 Task contract나 Evidence를 대체하지 않는다.
...
수용 기준은 나중에 Evidence와 대조하기 쉽도록 `AC-001:` 같은 안정적인 식별자를 붙여 쓸 수 있다. 이는 작성 관례이며 runtime schema를 바꾸는 규칙은 아니다.
...
|단계|기준 입력|남기는 것|다음으로 갈 수 있는 조건|멈추거나 돌아가는 조건|
|---|---|---|---|---|
|implementation|Task Contract|산출물, Implementation Evidence|자기 점검 뒤 전달 가능하다.|계약과 다른 점이 있거나 범위 밖 작업이 필요하다.|
|verification|Task Contract, 산출물, Implementation Evidence|Verification Evidence|필요한 확인을 수행했거나 미검증 범위를 드러낸다.|실패, 차단, 기준 부족, User 판단 필요가 드러난다.|
|review|산출물, Implementation Evidence, Verification Evidence|Review Evidence|review focus에 따른 점검을 마친다.|재작업 finding, 추가 verification 필요가 드러난다.|
|challenge|Task Contract, 산출물, Evidence|Challenger Evidence|숨은 가정과 위험을 압박한다.|Mission 또는 User 수준 판단이 필요하다.|
|수용 판단 대기|산출물, Evidence, Task Contract|User Judgment|User가 수용, 재작업, 보류, 중단 중 판단한다.|판단 보류나 기준선 갱신이 필요하다.|
...
Task는 Task State가 `closed`를 가리킨다는 이유만으로 완료된 것이 아니다. Task 결과에 대한 User Judgment가 남고, Task Evidence가 기준별 결과, 주요 Evidence 참조, User가 받아들인 미검증 범위와 남은 위험을 요약한 뒤에 닫힌 Task로 다룬다.
...
재작업은 User가 `revise`로 판단한 이유와 Evidence에서 드러난 실패 지점을 기준으로 돌아갈 위치를 정한다. 구현 변경이 필요하면 implementation으로 돌아가고, 구현 변경 없이 다시 확인하면 verification으로 돌아간다. 추가 품질 점검이 필요하면 review로, 숨은 위험 압박이 필요하면 challenge로 돌아간다. Task contract가 바뀌어야 하면 새 Task contract에 합의하고 기록한 뒤 필요한 phase로 이어 간다.
...
|상황|돌아갈 위치|남길 것|
|---|---|---|
|Task 추가, 삭제, dependency, Task contract의 범위, 산출물, 수용 기준, verification checks, review focus, 위험 수준이 바뀌어야 한다.|Task contract|바뀐 내용과 이유|
