...
- 초기 Task contract가 Mission을 판단 가능한 Task 단위로 나누는지 확인한다.
...
- Mission spec을 바탕으로 Mission design을 작성하고 User 검토로 올린다.
- Mission spec과 Mission design을 바탕으로 초기 Task contract를 작성하고 User 검토로 올린다.
...
- building 중 새 Task contract나 기존 Task contract 갱신이 필요하면 갱신안을 작성하고 User 수용 판단으로 연결한다.
...
- Mission design이나 Task contract를 User 승인 없이 조용히 확장할 수 없다.
...
- Implementer, Reviewer, Verifier, Challenger의 Evidence 작성 책임은 서로 섞지 않는다.
