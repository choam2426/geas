...
|Challenger|숨겨진 복잡성, 경계 위반, 모호한 계약, 검증 공백을 드러낸다.|

### 실행과 근거

|역할|핵심 책임|
|---|---|
|Implementer|승인된 Task contract에 따라 execution plan을 세우고 Task를 수행하며 self-check를 남긴다.|
...
- Task Evidence를 모아 User가 Task 수용 판단할 수 있는 형태로 정리한다.
...
Challenger는 devil's advocate 관점에서 Mission과 Task의 기준선을 흔들어 본다.

Challenger의 목적은 Mission spec, Mission design, Task contract가 과도하게 낙관적이거나 모호하거나 위험을 숨기고 있는지 드러내어, User가 더 낮은 비용으로 수용 판단할 수 있게 만드는 것이다.
...
- 필요한 경우 devil's advocate 관점의 검토를 수행하고 Challenger Evidence를 남긴다.
...
- 작업 결과, 변경 내용, 중요한 판단을 남긴다.
- 검토와 검증으로 넘기기 전에 self-check를 남긴다.

Implementer의 책임 경계는 다음과 같다.

- Task contract를 User 수용 판단 없이 조용히 확장할 수 없다.
- 자기 작업의 독립 review나 verification을 대신할 수 없다.
- self-check를 Review Evidence나 Verification Evidence처럼 표현할 수 없다.
