...
모든 작업에 범위, 수용 기준, 리뷰어, verification plan, 위험도, 라우팅 정책을 담은 계약을 먼저 만듭니다. 구현자는 코드를 쓰기 전에 implementation contract를 작성하고 리뷰어의 동의를 받습니다.
...
**칸반 보드** — task가 9단계 lifecycle 컬럼(drafted → ready → implementing → reviewing → deciding → passed, 측면 상태 blocked / escalated / cancelled)을 따라 이동합니다. 카드를 클릭하면 contract, evidence, closure 섹션을 확인할 수 있습니다.

![칸반 보드](docs/images/kanvanboard.png)

**미션 상세** — mission design, task 목록, gap analysis, debt ledger, mission verdict까지 프로토콜이 만든 모든 산출물을 한 화면에서 확인합니다.

**메모리 브라우저** — `shared.md`와 에이전트별 memory note를 열람합니다. 팀이 어떤 교훈을 쌓았는지 볼 수 있습니다.
