...
### 멀티 에이전트 팀 기반 개발에 질서를 부여하는 프로토콜
...
멀티 에이전트 개발은 빠르고 강력하지만, 통제 없이 돌리면 늘 같은 식으로 무너집니다:
...
- **Critical Reviewer** — 위험도가 높은 태스크는 별도 검증을 거침
...
[Arch Authority]   기술 가이드: bcrypt + JWT, refresh token rotation.
[Orchestrator]     구현 계약 승인됨.
[Backend Eng]      구현 완료. 4개 엔드포인트. worktree 병합.
[Backend Eng]      셀프체크: confidence 4/5. 토큰 만료 엣지케이스 미확인.
[Arch Authority]   코드 리뷰: 승인.                              <- 병렬 실행
[QA Engineer]      테스트: 수용 기준 6/6 통과.                    <- 병렬 실행
[Orchestrator]     Evidence Gate: PASS. Closure packet 조립됨.
[Critical Rev]     챌린지: rate limiting 미적용 [BLOCKING].
